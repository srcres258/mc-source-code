...
public class OggAudioStream implements AudioStream {
    ...
    public OggAudioStream(InputStream pInput) throws IOException {
        ...
        try (MemoryStack memorystack = MemoryStack.stackPush()) {
            ...
            while(this.handle == 0L) {
                ...
                this.handle = STBVorbis.stb_vorbis_open_pushdata(this.buffer, intbuffer, intbuffer1, null);
     ...
     * @return {@code true} if the buffer was successfully refilled, {@code false} if the end of the input stream was reached
    ...
    /**
     * Converts mono audio data from the specified channel to a byte buffer and appends it to the specified output buffer.
     * The method reads samples from the channel and adds them to the output buffer until there are no more samples remaining in the channel.
     *
     * @param pChannel the channel containing the mono audio data to convert
     * @param pOutput  the output buffer to which the converted audio data should be
     *                 appended
     */
    ...
    /**
     * Converts stereo audio data from the specified left and right channels to a byte buffer and appends it to the specified output buffer. The method reads samples from both channels and interleaves them in the output buffer until there are no more samples remaining in either channel.
     *
     ...
     * @param pOutput       the output buffer to which the converted audio data should
     *                      be appended
     */
    ...
    /**
     * Closes the audio stream. This method frees any resources associated with the stream, such as the handle for the Ogg Vorbis stream and the memory used to store the input buffer.
     ...
     * The method reads audio frames from the stream and adds them to the output buffer until the end of the stream is reached.
