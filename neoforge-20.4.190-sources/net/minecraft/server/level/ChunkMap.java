...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider {
    ...
    public ChunkMap(
        ...
        this.queueSorter = new ChunkTaskPriorityQueueSorter(
            ImmutableList.of(processormailbox1, processorhandle, processormailbox), pDispatcher, Integer.MAX_VALUE
        );
    ...
    public void debugReloadGenerator() {
        ...
        dataresult1.result().ifPresent(p_183808_ -> this.generator = p_183808_);
    }
    ...
    boolean isChunkTracked(ServerPlayer pPlayer, int pX, int pZ) {
        return pPlayer.getChunkTrackingView().contains(pX, pZ)
            && !pPlayer.connection.chunkSender.isPending(ChunkPos.asLong(pX, pZ));
    }
    ...
    @Nullable
    public ChunkHolder getVisibleChunkIfPresent(long pChunkPos) {
    ...
    public String getChunkDebugData(ChunkPos pPos) {
        ...
        if (chunkholder == null) {
            ...
            s = s + 167 + fullchunkstatus.ordinal() + fullchunkstatus;
            return s + "\u00a7r";
        }
    }

    private CompletableFuture<Either<List<ChunkAccess>, ChunkHolder.ChunkLoadingFailure>> getChunkRangeFuture(
        ChunkHolder pChunkHolder, int pRange, IntFunction<ChunkStatus> pStatusGetter
    ) {
        if (pRange == 0) {
            ChunkStatus chunkstatus1 = pStatusGetter.apply(0);
            return pChunkHolder.getOrScheduleFuture(chunkstatus1, this).thenApply(p_214893_ -> p_214893_.mapLeft(List::of));
        } else {
            ...
            for(int k = -pRange; k <= pRange; ++k) {
                for(int l = -pRange; l <= pRange; ++l) {
                    ...
                    CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> completablefuture = chunkholder.getOrScheduleFuture(
                        chunkstatus, this
                    );
    ...
    public CompletableFuture<Either<LevelChunk, ChunkHolder.ChunkLoadingFailure>> prepareEntityTickingChunk(ChunkHolder pChunk) {
        return this.getChunkRangeFuture(pChunk, 2, p_203078_ -> ChunkStatus.FULL)
            .thenApplyAsync(p_212878_ -> p_212878_.mapLeft(p_214939_ -> (LevelChunk)p_214939_.get(p_214939_.size() / 2)), this.mainThreadExecutor);
    }

    /**
     * Sets level and loads/unloads chunk.
     *
     * @param pHolder The {@link net.minecraft.server.level.ChunkHolder} of the chunk
    ...
    ChunkHolder updateChunkScheduling(long pChunkPos, int pNewLevel, @Nullable ChunkHolder pHolder, int pOldLevel) {
        if (!ChunkLevel.isLoaded(pOldLevel) && !ChunkLevel.isLoaded(pNewLevel)) {
            ...
            net.neoforged.neoforge.event.EventHooks.fireChunkTicketLevelUpdated(this.level, pChunkPos, pOldLevel, pNewLevel, pHolder);
            return pHolder;
        }
    }
        ...
        if (pStatus == ChunkStatus.EMPTY) {
            ...
            if (!pStatus.hasLoadDependencies()) {
                ...
                if (optional.isPresent() && optional.get().getStatus().isOrAfter(pStatus)) {
                    CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> completablefuture = pStatus.load(
                        this.level, this.structureTemplateManager, this.lightEngine, p_203081_ -> this.protoChunkToFullChunk(pHolder), optional.get()
                    );
    ...
    private CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> scheduleChunkLoad(ChunkPos pChunkPos) {
        return this.readChunk(pChunkPos).thenApply(p_214925_ -> p_214925_.filter(p_214928_ -> {
            ...
            if (p_313584_.isPresent()) {
                ChunkAccess chunkaccess = ChunkSerializer.read(this.level, this.poiManager, pChunkPos, p_313584_.get());
        ...
        }, this.mainThreadExecutor).exceptionallyAsync(p_214888_ -> this.handleChunkLoadFailure(p_214888_, pChunkPos), this.mainThreadExecutor);
    }
    ...
    private CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> scheduleChunkGeneration(ChunkHolder pChunkHolder, ChunkStatus pChunkStatus) {
        ...
        CompletableFuture<Either<List<ChunkAccess>, ChunkHolder.ChunkLoadingFailure>> completablefuture = this.getChunkRangeFuture(
            pChunkHolder, pChunkStatus.getRange(), p_214935_ -> this.getDependencyStatus(pChunkStatus, p_214935_)
        );
                    ...
                    p_280971_ -> {
                        try {
                            ...
                            if (chunkaccess.getStatus().isOrAfter(pChunkStatus)) {
                                completablefuture1 = pChunkStatus.load(
                                    this.level,
                                    this.structureTemplateManager,
                                    this.lightEngine,
                                    p_214919_ -> this.protoChunkToFullChunk(pChunkHolder),
                                    chunkaccess
                                );
                            } else {
                                completablefuture1 = pChunkStatus.generate(
                                    executor,
                                    this.level,
                                    this.generator,
                                    this.structureTemplateManager,
                                    this.lightEngine,
                                    p_280966_ -> this.protoChunkToFullChunk(pChunkHolder),
                                    p_280971_
                                );
                            }
    ...
    private CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> protoChunkToFullChunk(ChunkHolder pHolder) {
        CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> completablefuture = pHolder.getFutureIfPresentUnchecked(
    ...
    public CompletableFuture<Either<LevelChunk, ChunkHolder.ChunkLoadingFailure>> prepareTickingChunk(ChunkHolder pHolder) {
        CompletableFuture<Either<List<ChunkAccess>, ChunkHolder.ChunkLoadingFailure>> completablefuture = this.getChunkRangeFuture(
            pHolder, 1, p_214916_ -> ChunkStatus.FULL
        );
        CompletableFuture<Either<LevelChunk, ChunkHolder.ChunkLoadingFailure>> completablefuture1 = completablefuture.<Either<LevelChunk, ChunkHolder.ChunkLoadingFailure>>thenApplyAsync(
                p_280964_ -> p_280964_.mapLeft(p_293806_ -> (LevelChunk)p_293806_.get(p_293806_.size() / 2)),
                p_214944_ -> this.mainThreadMailbox.tell(ChunkTaskPriorityQueueSorter.message(pHolder, p_214944_))
            )
            .thenApplyAsync(p_300772_ -> p_300772_.ifLeft(p_300770_ -> {
    ...
    public CompletableFuture<Either<LevelChunk, ChunkHolder.ChunkLoadingFailure>> prepareAccessibleChunk(ChunkHolder pHolder) {
        return this.getChunkRangeFuture(pHolder, 1, ChunkStatus::getStatusAroundFullChunk)
            .thenApplyAsync(
                p_203086_ -> p_203086_.mapLeft(p_203092_ -> (LevelChunk)p_203092_.get(p_203092_.size() / 2)),
    ...
    private static void markChunkPendingToSend(ServerPlayer pPlayer, LevelChunk pChunk) {
        ...
        net.neoforged.neoforge.event.EventHooks.fireChunkWatch(pPlayer, pChunk, pPlayer.serverLevel());
    }

    private static void dropChunk(ServerPlayer pPlayer, ChunkPos pChunkPos) {
        net.neoforged.neoforge.event.EventHooks.fireChunkUnWatch(pPlayer, pChunkPos, pPlayer.serverLevel());
    ...
    private static String printFuture(CompletableFuture<Either<LevelChunk, ChunkHolder.ChunkLoadingFailure>> pFuture) {
        try {
            Either<LevelChunk, ChunkHolder.ChunkLoadingFailure> either = pFuture.getNow(null);
            return either != null ? either.map(p_214897_ -> "done", p_214850_ -> "unloaded") : "not completed";
    ...
    private void applyChunkTrackingView(ServerPlayer pPlayer, ChunkTrackingView pChunkTrackingView) {
        if (pPlayer.level() == this.level) {
            ...
            ChunkTrackingView.difference(
                chunktrackingview, pChunkTrackingView, p_293802_ -> this.markChunkPendingToSend(pPlayer, p_293802_), p_293800_ -> dropChunk(pPlayer, p_293800_)
            );
    ...
    protected void addEntity(Entity pEntity) {
        if (!(pEntity instanceof net.neoforged.neoforge.entity.PartEntity)) {
    ...
    public void waitForLightBeforeSending(ChunkPos pChunkPos, int p_301130_) {
        int i = p_301130_ + 1;
        ChunkPos.rangeClosed(pChunkPos, i).forEach(p_300775_ -> {
