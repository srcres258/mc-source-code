...
public class SpreadPlayersCommand {
                                            ...
                                            Commands.argument("respectTeams", BoolArgumentType.bool())
                                                .then(
                                                    Commands.argument("targets", EntityArgument.entities())
                                                        .executes(
                                                            p_313567_ -> spreadPlayers(
                                                                    p_313567_.getSource(),
                                                                    Vec2Argument.getVec2(p_313567_, "center"),
                                                                    FloatArgumentType.getFloat(p_313567_, "spreadDistance"),
                                                                    FloatArgumentType.getFloat(p_313567_, "maxRange"),
                                                                    p_313567_.getSource().getLevel().getMaxBuildHeight(),
                                                                    BoolArgumentType.getBool(p_313567_, "respectTeams"),
                                                                    EntityArgument.getEntities(p_313567_, "targets")
                                                                )
                                                        )
                                                )
                                        )
    ...
    private static double setPlayerPositions(
        ...
        for(Entity entity : pTargets) {
            ...
            net.neoforged.neoforge.event.entity.EntityTeleportEvent.SpreadPlayersCommand event = net.neoforged.neoforge.event.EventHooks.onEntityTeleportSpreadPlayersCommand(entity,
                      ...
                      spreadplayerscommand$position.getSpawnY(pLevel, pMaxHeight),
    ...
    static class Position {
        ...
        public int getSpawnY(BlockGetter pLevel, int pY) {
            ...
            boolean flag2;
            for(boolean flag1 = pLevel.getBlockState(blockpos$mutableblockpos).isAir();
                blockpos$mutableblockpos.getY() > pLevel.getMinBuildHeight();
                flag1 = flag2
            ) {
                blockpos$mutableblockpos.move(Direction.DOWN);
