...
public class WorldBorderCommand {
                        ...
                        .then(
                            Commands.argument("distance", DoubleArgumentType.doubleArg(-5.999997E7F, 5.999997E7F))
                                .executes(
                                    p_309503_ -> setSize(
                                            p_309503_.getSource(),
                                            p_309503_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_309503_, "distance"),
                                            0L
                                        )
                                )
                                .then(
                                    Commands.argument("time", IntegerArgumentType.integer(0))
                                        .executes(
                                            p_309502_ -> setSize(
                                                    p_309502_.getSource(),
                                                    p_309502_.getSource().getLevel().getWorldBorder().getSize()
                                                        + DoubleArgumentType.getDouble(p_309502_, "distance"),
                                                    p_309502_.getSource().getLevel().getWorldBorder().getLerpRemainingTime()
                                                        + (long)IntegerArgumentType.getInteger(p_309502_, "time") * 1000L
                                                )
                                        )
                                )
                        )
    ...
    private static int setDamageBuffer(CommandSourceStack pSource, float pDistance) throws CommandSyntaxException {
        ...
        if (worldborder.getDamageSafeZone() == (double)pDistance) {
            ...
            pSource.sendSuccess(
                () -> Component.translatable("commands.worldborder.damage.buffer.success", String.format(Locale.ROOT, "%.2f", pDistance)), true
            );
    ...
    private static int setDamageAmount(CommandSourceStack pSource, float pDamagePerBlock) throws CommandSyntaxException {
        ...
        if (worldborder.getDamagePerBlock() == (double)pDamagePerBlock) {
            ...
            pSource.sendSuccess(
                () -> Component.translatable("commands.worldborder.damage.amount.success", String.format(Locale.ROOT, "%.2f", pDamagePerBlock)), true
            );
        ...
        if (worldborder.getCenterX() == (double)pPos.x && worldborder.getCenterZ() == (double)pPos.y) {
            ...
            pSource.sendSuccess(
                () -> Component.translatable(
                        "commands.worldborder.center.success", String.format(Locale.ROOT, "%.2f", pPos.x), String.format(Locale.ROOT, "%.2f", pPos.y)
                    ),
                true
            );
            return 0;
