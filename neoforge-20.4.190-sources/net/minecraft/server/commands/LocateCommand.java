...
public class LocateCommand {
    ...
    private static Optional<? extends HolderSet.ListBacked<Structure>> getHolders(
        ...
        return pStructure.unwrap().map(p_258231_ -> pStructureRegistry.getHolder(p_258231_).map(p_214491_ -> HolderSet.direct(p_214491_)), pStructureRegistry::getTag);
    }
    ...
    private static int locatePoi(CommandSourceStack pSource, ResourceOrTagArgument.Result<PoiType> pPoiType) throws CommandSyntaxException {
        ...
        Optional<Pair<Holder<PoiType>, BlockPos>> optional = serverlevel.getPoiManager()
            .findClosestWithType(pPoiType, blockpos, 256, PoiManager.Occupancy.ANY);
        stopwatch.stop();
        if (optional.isEmpty()) {
    ...
    private static String getElementName(Pair<BlockPos, ? extends Holder<?>> pResultWithPosition) {
        return pResultWithPosition.getSecond().unwrapKey().map(p_214498_ -> p_214498_.location().toString()).orElse("[unregistered]");
    }
    ...
    public static int showLocateResult(
        ...
        String s = pResult.unwrap().map(p_248147_ -> pResult.asPrintable(), p_248143_ -> pResult.asPrintable() + " (" + getElementName(pResultWithPosition) + ")");
    ...
    public static int showLocateResult(
        ...
        String s = pResult.unwrap()
            .map(p_214463_ -> p_214463_.location().toString(), p_248145_ -> "#" + p_248145_.location() + " (" + getElementName(pResultWithPosition) + ")");
        return showLocateResult(pSource, pSourcePosition, pResultWithPosition, pTranslationKey, pAbsoluteY, s, pDuration);
    }
