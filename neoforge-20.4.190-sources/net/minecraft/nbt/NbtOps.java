...
public class NbtOps implements DynamicOps<Tag> {
    ...
    public DataResult<Tag> mergeToMap(Tag pMap, Tag pKey, Tag pValue) {
        if (!(pMap instanceof CompoundTag) && !(pMap instanceof EndTag)) {
            ...
            if (pMap instanceof CompoundTag compoundtag1) {
                compoundtag1.getAllKeys().forEach(p_129068_ -> compoundtag.put(p_129068_, compoundtag1.get(p_129068_)));
            }
    ...
    public DataResult<Tag> mergeToMap(Tag pMap, MapLike<Tag> pOtherMap) {
        if (!(pMap instanceof CompoundTag) && !(pMap instanceof EndTag)) {
            ...
            CompoundTag compoundtag = new CompoundTag();
            if (pMap instanceof CompoundTag compoundtag1) {
                compoundtag1.getAllKeys().forEach(p_129059_ -> compoundtag.put(p_129059_, compoundtag1.get(p_129059_)));
            }
    ...
    public DataResult<Consumer<BiConsumer<Tag, Tag>>> getMapEntries(Tag pMap) {
        return pMap instanceof CompoundTag compoundtag
            ? DataResult.success(
                p_129024_ -> compoundtag.getAllKeys().forEach(p_178006_ -> p_129024_.accept(this.createString(p_178006_), compoundtag.get(p_178006_)))
            )
            : DataResult.error(() -> "Not a map: " + pMap);
    }
    ...
    public DataResult<MapLike<Tag>> getMap(Tag pMap) {
        return pMap instanceof CompoundTag compoundtag ? DataResult.success(new MapLike<Tag>() {
            ...
            public Stream<Pair<Tag, Tag>> entries() {
                return compoundtag.getAllKeys().stream().map(p_129172_ -> Pair.of(NbtOps.this.createString(p_129172_), compoundtag.get(p_129172_)));
            }
    ...
    public DataResult<Consumer<Consumer<Tag>>> getList(Tag pTag) {
        if (pTag instanceof ListTag listtag) {
            return listtag.getElementType() == 10
                ? DataResult.success(p_248055_ -> listtag.forEach(p_248051_ -> p_248055_.accept(tryUnwrap((CompoundTag)p_248051_))))
    ...
    public Tag remove(Tag pMap, String pRemoveKey) {
        if (pMap instanceof CompoundTag compoundtag) {
            ...
            compoundtag.getAllKeys()
                .stream()
                .filter(p_128988_ -> !Objects.equals(p_128988_, pRemoveKey))
                .forEach(p_129028_ -> compoundtag1.put(p_129028_, compoundtag.get(p_129028_)));
            return compoundtag1;
        } else {
            return pMap;
        }
    ...
    class NbtRecordBuilder extends AbstractStringBuilder<Tag, CompoundTag> {
        ...
        protected DataResult<Tag> build(CompoundTag p_129190_, Tag p_129191_) {
            ...
            } else if (!(p_129191_ instanceof CompoundTag)) {
