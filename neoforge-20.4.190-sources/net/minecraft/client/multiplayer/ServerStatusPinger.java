                ...
                public void handleStatusResponse(ClientboundStatusResponsePacket p_105489_) {
                    if (this.receivedPing) {
                        ...
                        serverstatus.favicon().ifPresent(p_302312_ -> {
                            if (!Arrays.equals(p_302312_.iconBytes(), pServer.getIconBytes())) {
                                pServer.setIconBytes(ServerData.validateIcon(p_302312_.iconBytes()));
    ...
    void pingLegacyServer(InetSocketAddress pResolvedServerAddress, final ServerAddress pServerAddress, final ServerData pServerData) {
        new Bootstrap().group(Connection.NETWORK_WORKER_GROUP.get()).handler(new ChannelInitializer<Channel>() {
            ...
            protected void initChannel(Channel p_105498_) {
                ...
                p_105498_.pipeline().addLast(new LegacyServerPinger(pServerAddress, (p_295082_, p_296021_, p_294466_, p_295675_, p_294359_) -> {
                    pServerData.protocol = -1;
                    pServerData.version = Component.literal(p_296021_);
                    pServerData.motd = Component.literal(p_294466_);
                    pServerData.status = ServerStatusPinger.formatPlayerCount(p_295675_, p_294359_);
                    pServerData.players = new ServerStatus.Players(p_294359_, p_295675_, List.of());
                }));
            }
