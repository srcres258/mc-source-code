...
public class FontManager implements PreparableReloadListener, AutoCloseable {
    ...
    public FontManager(TextureManager pTextureManager) {
        ...
        this.missingFontSet = Util.make(new FontSet(pTextureManager, MISSING_FONT), p_95010_ -> p_95010_.reload(Lists.newArrayList(new AllMissingGlyphProvider())));
    }
    ...
    public CompletableFuture<Void> reload(
        ...
        return this.prepare(pResourceManager, pBackgroundExecutor).thenCompose(pPreparationBarrier::wait).thenAcceptAsync(p_284609_ -> this.apply(p_284609_, pReloadProfiler), pGameExecutor);
    }
    ...
    private CompletableFuture<FontManager.Preparation> prepare(ResourceManager pResourceManager, Executor pExecutor) {
        ...
        for(Entry<ResourceLocation, List<Resource>> entry : FONT_DEFINITIONS.listMatchingResourceStacks(pResourceManager).entrySet()) {
            ...
            list.add(CompletableFuture.supplyAsync(() -> {
                ...
                for(Pair<FontManager.BuilderId, GlyphProviderDefinition> pair : list1) {
                    ...
                    }).ifRight(p_286129_ -> fontmanager$unresolvedbuilderbundle.add(fontmanager$builderid, p_286129_));
                }
        ...
        return Util.sequence(list)
            .thenCompose(
                p_284592_ -> {
                    List<CompletableFuture<Optional<GlyphProvider>>> list1 = p_284592_.stream()
                        ...
                        .thenCompose(
                            p_284618_ -> {
                                ...
                                CompletableFuture<?>[] completablefuture = map.values()
                                    .stream()
                                    .map(p_284585_ -> CompletableFuture.runAsync(() -> this.finalizeProviderLoading(p_284585_, glyphprovider), pExecutor))
                                    .toArray(p_284587_ -> new CompletableFuture[p_284587_]);
    ...
    private Map<ResourceLocation, List<GlyphProvider>> resolveProviders(List<FontManager.UnresolvedBuilderBundle> pUnresolvedBuilderBundles) {
        ...
        dependencysorter.orderByDependencies((p_284620_, p_284621_) -> p_284621_.resolve(map::get).ifPresent(p_284590_ -> map.put(p_284620_, p_284590_)));
        return map;
    }
    ...
    private static List<Pair<FontManager.BuilderId, GlyphProviderDefinition>> loadResourceStack(List<Resource> pResources, ResourceLocation pFontId) {
        ...
        for(Resource resource : pResources) {
            try (Reader reader = resource.openAsReader()) {
                ...
                FontManager.FontDefinitionFile fontmanager$fontdefinitionfile = Util.getOrThrow(
    ...
    public Font createFont() {
        return new Font(p_284586_ -> this.fontSets.getOrDefault(this.getActualId(p_284586_), this.missingFontSet), false);
    }

    public Font createFontFilterFishy() {
        return new Font(p_284596_ -> this.fontSets.getOrDefault(this.getActualId(p_284596_), this.missingFontSet), true);
    }
    ...
    static record BuilderResult(FontManager.BuilderId id, Either<CompletableFuture<Optional<GlyphProvider>>, ResourceLocation> result) {
        public Optional<List<GlyphProvider>> resolve(Function<ResourceLocation, List<GlyphProvider>> pProviderResolver) {
            return this.result
                .map(
                    p_285332_ -> p_285332_.join().map(List::of),
                    p_285367_ -> {
    ...
    static record FontDefinitionFile(List<GlyphProviderDefinition> providers) {
        public static final Codec<FontManager.FontDefinitionFile> CODEC = RecordCodecBuilder.create(
            p_286425_ -> p_286425_.group(GlyphProviderDefinition.CODEC.listOf().fieldOf("providers").forGetter(FontManager.FontDefinitionFile::providers))
                    .apply(p_286425_, FontManager.FontDefinitionFile::new)
        );
    }
