...
public class ClientRecipeBook extends RecipeBook {
    ...
    public void setupCollections(Iterable<RecipeHolder<?>> pRecipes, RegistryAccess pRegistryAccess) {
        Map<RecipeBookCategories, List<List<RecipeHolder<?>>>> map = categorizeAndGroupRecipes(pRecipes);
        ...
        map.forEach(
            (p_266602_, p_266603_) -> map1.put(
                    p_266602_,
                    p_266603_.stream().map(p_266605_ -> new RecipeCollection(pRegistryAccess, p_266605_)).peek(builder::add).collect(ImmutableList.toImmutableList())
                )
        );
    ...
    private static Map<RecipeBookCategories, List<List<RecipeHolder<?>>>> categorizeAndGroupRecipes(Iterable<RecipeHolder<?>> pRecipes) {
    ...
    private static RecipeBookCategories getCategory(RecipeHolder<?> pRecipe) {
        Recipe<?> recipe = pRecipe.value();
        if (recipe instanceof CraftingRecipe craftingrecipe) {
            ...
            if (recipetype == RecipeType.STONECUTTING) {
                ...
                RecipeBookCategories categories = net.neoforged.neoforge.client.RecipeBookManager.findCategories((RecipeType) recipetype, pRecipe);
                if (categories != null) return categories;
                LOGGER.warn(
                    "Unknown recipe category: {}/{}",
                    LogUtils.defer(() -> BuiltInRegistries.RECIPE_TYPE.getKey(recipe.getType())),
                    LogUtils.defer(pRecipe::id)
                );
