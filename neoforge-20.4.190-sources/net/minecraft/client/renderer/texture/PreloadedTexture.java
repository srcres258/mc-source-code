...
public class PreloadedTexture extends SimpleTexture {
    ...
    public PreloadedTexture(ResourceManager pResourceManager, ResourceLocation pLocation, Executor pBackgroundExecutor) {
        super(pLocation);
        this.future = CompletableFuture.supplyAsync(() -> SimpleTexture.TextureImage.load(pResourceManager, pLocation), pBackgroundExecutor);
    }
    ...
    public void reset(TextureManager pTextureManager, ResourceManager pResourceManager, ResourceLocation pResourceLocation, Executor pExecutor) {
        this.future = CompletableFuture.supplyAsync(() -> SimpleTexture.TextureImage.load(pResourceManager, this.location), Util.backgroundExecutor());
        this.future.thenRunAsync(() -> pTextureManager.register(this.location, this), executor(pExecutor));
    }

    private static Executor executor(Executor pExecutor) {
        return p_118124_ -> pExecutor.execute(() -> RenderSystem.recordRenderCall(p_118124_::run));
    }
}
