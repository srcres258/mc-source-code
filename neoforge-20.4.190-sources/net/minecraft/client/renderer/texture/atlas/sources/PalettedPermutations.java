...
public class PalettedPermutations implements SpriteSource {
    ...
    public void run(ResourceManager pResourceManager, SpriteSource.Output pOutput) {
        Supplier<int[]> supplier = Suppliers.memoize(() -> loadPaletteEntryFromImage(pResourceManager, this.paletteKey));
        Map<String, Supplier<IntUnaryOperator>> map = new HashMap<>();
        this.permutations
            .forEach(
                (p_267108_, p_266969_) -> map.put(
                        p_267108_, Suppliers.memoize(() -> createPaletteMapping(supplier.get(), loadPaletteEntryFromImage(pResourceManager, p_266969_)))
                    )
            );
