...
public class SpriteContents implements Stitcher.Entry, AutoCloseable {
    ...
    public SpriteContents(ResourceLocation pName, FrameSize pFrameSize, NativeImage pOriginalImage, ResourceMetadata pMetadata) {
        ...
        AnimationMetadataSection animationmetadatasection = pMetadata.getSection(AnimationMetadataSection.SERIALIZER).orElse(AnimationMetadataSection.EMPTY);
        this.animatedTexture = this.createAnimatedTexture(pFrameSize, pOriginalImage.getWidth(), pOriginalImage.getHeight(), animationmetadatasection);
    ...
    private SpriteContents.AnimatedTexture createAnimatedTexture(FrameSize pFrameSize, int pWidth, int pHeight, AnimationMetadataSection pMetadata) {
        ...
        pMetadata.forEachFrame((p_251291_, p_251837_) -> list.add(new SpriteContents.FrameInfo(p_251291_, p_251837_)));
        if (list.isEmpty()) {
            for(int l = 0; l < k; ++l) {
    ...
    void upload(int pX, int pY, int pFrameX, int pFrameY, NativeImage[] pAtlasData) {
        for(int i = 0; i < this.byMipLevel.length; ++i) {
            ...
            pAtlasData[i]
                .upload(i, pX >> i, pY >> i, pFrameX >> i, pFrameY >> i, this.width >> i, this.height >> i, this.byMipLevel.length > 1, false);
        }
    }
    ...
    final class InterpolationData implements AutoCloseable {
        ...
        void uploadInterpolatedFrame(int pX, int pY, SpriteContents.Ticker pTicker) {
            ...
            double d0 = 1.0 - (double)pTicker.subFrame / (double)spritecontents$frameinfo.time;
        ...
        private int mix(double pDelta, int pColor1, int pColor2) {
            return (int)(pDelta * (double)pColor1 + (1.0 - pDelta) * (double)pColor2);
        }
        ...
        public void tickAndUpload(int pX, int pY) {
            ...
            if (this.subFrame >= spritecontents$frameinfo.time) {
                ...
                if (!RenderSystem.isOnRenderThread()) {
                    RenderSystem.recordRenderCall(() -> this.interpolationData.uploadInterpolatedFrame(pX, pY, this));
                } else {
