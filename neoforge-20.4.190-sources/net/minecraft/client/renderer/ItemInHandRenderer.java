...
public class ItemInHandRenderer {
    ...
    private void renderOneHandedMap(
        PoseStack pPoseStack, MultiBufferSource pBuffer, int pCombinedLight, float pEquippedProgress, HumanoidArm pHand, float pSwingProgress, ItemStack pStack
    ) {
    ...
    public void renderHandsWithItems(float pPartialTicks, PoseStack pPoseStack, MultiBufferSource.BufferSource pBuffer, LocalPlayer pPlayerEntity, int pCombinedLight) {
        ...
        if (iteminhandrenderer$handrenderselection.renderMainHand) {
            ...
            if(!net.neoforged.neoforge.client.ClientHooks.renderSpecificFirstPersonHand(InteractionHand.MAIN_HAND, pPoseStack, pBuffer, pCombinedLight, pPartialTicks, f1, f4, f5, this.mainHandItem))
        ...
        if (iteminhandrenderer$handrenderselection.renderOffHand) {
            ...
            if(!net.neoforged.neoforge.client.ClientHooks.renderSpecificFirstPersonHand(InteractionHand.OFF_HAND, pPoseStack, pBuffer, pCombinedLight, pPartialTicks, f1, f6, f7, this.offHandItem))
    ...
    private void renderArmWithItem(
        ...
        if (!pPlayer.isScoping()) {
            ...
            if (pStack.isEmpty()) {
                ...
                if (!net.neoforged.neoforge.client.extensions.common.IClientItemExtensions.of(pStack).applyForgeHandTransform(pPoseStack, minecraft.player, humanoidarm, pStack, pPartialTicks, pEquippedProgress, pSwingProgress)) // FORGE: Allow items to define custom arm animation
                if (pPlayer.isUsingItem() && pPlayer.getUseItemRemainingTicks() > 0 && pPlayer.getUsedItemHand() == pHand) {
                    int k = flag3 ? 1 : -1;
