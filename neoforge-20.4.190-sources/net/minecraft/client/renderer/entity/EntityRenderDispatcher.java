...
public class EntityRenderDispatcher implements ResourceManagerReloadListener {
    ...
    private static void renderHitbox(PoseStack pPoseStack, VertexConsumer pBuffer, Entity pEntity, float pPartialTicks) {
        ...
        if (pEntity.isMultipartEntity()) {
            ...
            for(net.neoforged.neoforge.entity.PartEntity<?> enderdragonpart : pEntity.getParts()) {
                pPoseStack.pushPose();
        ...
        pBuffer.vertex(matrix4f, 0.0F, pEntity.getEyeHeight(), 0.0F)
            .color(0, 0, 255, 255)
            .normal(matrix3f, (float)vec31.x, (float)vec31.y, (float)vec31.z)
            .endVertex();
        pBuffer.vertex(matrix4f, (float)(vec31.x * 2.0), (float)((double)pEntity.getEyeHeight() + vec31.y * 2.0), (float)(vec31.z * 2.0))
            .color(0, 0, 255, 255)
            .normal(matrix3f, (float)vec31.x, (float)vec31.y, (float)vec31.z)
            .endVertex();
    }
    ...
    private static void renderShadow(
        ...
        float f = pSize;
        if (pEntity instanceof Mob mob && mob.isBaby()) {
            f = pSize * 0.5F;
        }
