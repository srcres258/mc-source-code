...
public class WitherBossModel<T extends WitherBoss> extends HierarchicalModel<T> {
    ...
    public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        this.centerHead.yRot = pNetHeadYaw * (float) (Math.PI / 180.0);
        this.centerHead.xRot = pHeadPitch * (float) (Math.PI / 180.0);
    }
    ...
    private static <T extends WitherBoss> void setupHeadRotation(T pWither, ModelPart pPart, int pHead) {
        pPart.yRot = (pWither.getHeadYRot(pHead) - pWither.yBodyRot) * (float) (Math.PI / 180.0);
        pPart.xRot = pWither.getHeadXRot(pHead) * (float) (Math.PI / 180.0);
    }
}
