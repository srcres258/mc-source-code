...
public class StriderModel<T extends Strider> extends HierarchicalModel<T> {
    ...
    public void setupAnim(Strider pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        if (!pEntity.isVehicle()) {
            this.body.xRot = pHeadPitch * (float) (Math.PI / 180.0);
            this.body.yRot = pNetHeadYaw * (float) (Math.PI / 180.0);
        } else {
            this.body.xRot = 0.0F;
            this.body.yRot = 0.0F;
        }
        ...
        this.leftLeg.zRot = (float) (Math.PI / 18) * Mth.cos(pLimbSwing * 1.5F * 0.5F) * pLimbSwingAmount;
        this.rightLeg.zRot = (float) (Math.PI / 18) * Mth.cos(pLimbSwing * 1.5F * 0.5F + (float) Math.PI) * pLimbSwingAmount;
