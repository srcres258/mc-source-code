...
public class ArmorStandModel extends ArmorStandArmorModel {
    ...
    public void prepareMobModel(ArmorStand pEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTick) {
        ...
        this.basePlate.yRot = (float) (Math.PI / 180.0) * -Mth.rotLerp(pPartialTick, pEntity.yRotO, pEntity.getYRot());
    ...
    public void setupAnim(ArmorStand pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        this.rightBodyStick.xRot = (float) (Math.PI / 180.0) * pEntity.getBodyPose().getX();
        this.rightBodyStick.yRot = (float) (Math.PI / 180.0) * pEntity.getBodyPose().getY();
        this.rightBodyStick.zRot = (float) (Math.PI / 180.0) * pEntity.getBodyPose().getZ();
        this.leftBodyStick.xRot = (float) (Math.PI / 180.0) * pEntity.getBodyPose().getX();
        this.leftBodyStick.yRot = (float) (Math.PI / 180.0) * pEntity.getBodyPose().getY();
        this.leftBodyStick.zRot = (float) (Math.PI / 180.0) * pEntity.getBodyPose().getZ();
        this.shoulderStick.xRot = (float) (Math.PI / 180.0) * pEntity.getBodyPose().getX();
        this.shoulderStick.yRot = (float) (Math.PI / 180.0) * pEntity.getBodyPose().getY();
        this.shoulderStick.zRot = (float) (Math.PI / 180.0) * pEntity.getBodyPose().getZ();
    }
