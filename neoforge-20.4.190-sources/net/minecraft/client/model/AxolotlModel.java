...
public class AxolotlModel<T extends Axolotl & LerpingModel> extends AgeableListModel<T> {
    ...
    private void setupInitialAnimationValues(T pAxolotl, float pNetHeadYaw, float pHeadPitch) {
        ...
        Map<String, Vector3f> map = pAxolotl.getModelRotationValues();
        if (map.isEmpty()) {
            this.body.setRotation(pHeadPitch * (float) (Math.PI / 180.0), pNetHeadYaw * (float) (Math.PI / 180.0), 0.0F);
    ...
    private void setupLayStillOnGroundAnimation(float pAgeInTicks, float pNetHeadYaw) {
        ...
        this.body.yRot = this.lerpTo(this.body.yRot, pNetHeadYaw * (float) (Math.PI / 180.0));
    ...
    private void setupSwimmingAnimation(float pAgeInTicks, float pHeadPitch) {
        ...
        float f2 = Mth.cos(f);
        float f3 = 0.13F * f1;
        this.body.xRot = this.lerpTo(0.1F, this.body.xRot, pHeadPitch * (float) (Math.PI / 180.0) + f3);
