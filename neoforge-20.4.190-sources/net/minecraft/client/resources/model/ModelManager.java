...
public class ModelManager implements PreparableReloadListener, AutoCloseable {
    ...
    public final CompletableFuture<Void> reload(
        ...
        CompletableFuture<ModelBakery> completablefuture2 = completablefuture.thenCombineAsync(
            completablefuture1, (p_251201_, p_251281_) -> new ModelBakery(this.blockColors, pPreparationsProfiler, p_251201_, p_251281_), pBackgroundExecutor
        );
        ...
        return CompletableFuture.allOf(
                Stream.concat(map.values().stream(), Stream.of(completablefuture2)).toArray(p_250157_ -> new CompletableFuture[p_250157_])
            )
            .thenApplyAsync(
                p_248624_ -> this.loadModels(
                        ...
                        map.entrySet()
                            .stream()
                            .collect(Collectors.toMap(Entry::getKey, p_248988_ -> (AtlasSet.StitchResult)((CompletableFuture)p_248988_.getValue()).join())),
                        completablefuture2.join()
                    ),
                pBackgroundExecutor
            )
            .thenCompose(p_252255_ -> p_252255_.readyForUpload.thenApply(p_251581_ -> p_252255_))
    ...
    private static CompletableFuture<Map<ResourceLocation, List<ModelBakery.LoadedJson>>> loadBlockStates(ResourceManager pResourceManager, Executor pExecutor) {
        return CompletableFuture.<Map<ResourceLocation, List<Resource>>>supplyAsync(
                () -> ModelBakery.BLOCKSTATE_LISTER.listMatchingResourceStacks(pResourceManager), pExecutor
            )
            .thenCompose(
                p_250744_ -> {
    ...
    private ModelManager.ReloadState loadModels(ProfilerFiller pProfilerFiller, Map<ResourceLocation, AtlasSet.StitchResult> pAtlasPreparations, ModelBakery pModelBakery) {
            ...
            .forEach(
                (p_250493_, p_252017_) -> LOGGER.warn(
                        ...
                        p_252017_.stream()
                            ...
                            .map(p_248692_ -> "    " + p_248692_.atlasLocation() + ":" + p_248692_.texture())
                            .collect(Collectors.joining("\n"))
                    )
            );
        pProfilerFiller.popPush("forge_modify_baking_result");
        net.neoforged.neoforge.client.ClientHooks.onModifyBakingResult(pModelBakery.getBakedTopLevelModels(), pAtlasPreparations, pModelBakery);
        ...
        CompletableFuture<Void> completablefuture = CompletableFuture.allOf(
            pAtlasPreparations.values().stream().map(AtlasSet.StitchResult::readyForUpload).toArray(p_250044_ -> new CompletableFuture[p_250044_])
        );
        pProfilerFiller.pop();
