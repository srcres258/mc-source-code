...
public class TagManager implements PreparableReloadListener {
    ...
    public static String getTagDir(ResourceKey<? extends Registry<?>> pRegistryKey) {
        ...
        return s != null ? s : "tags/" + net.neoforged.neoforge.common.CommonHooks.prefixNamespace(pRegistryKey.location());
    }
    ...
    public CompletableFuture<Void> reload(
        ...
        List<? extends CompletableFuture<? extends TagManager.LoadResult<?>>> list = this.registryAccess
            .registries()
            .map(p_203927_ -> this.createLoader(pResourceManager, pBackgroundExecutor, p_203927_))
            .toList();
        return CompletableFuture.allOf(list.toArray(p_203906_ -> new CompletableFuture[p_203906_]))
    ...
    private <T> CompletableFuture<TagManager.LoadResult<T>> createLoader(
        ResourceManager pResourceManager, Executor pBackgroundExecutor, RegistryAccess.RegistryEntry<T> pEntry
    ) {
        ResourceKey<? extends Registry<T>> resourcekey = pEntry.key();
        Registry<T> registry = pEntry.value();
        TagLoader<Holder<T>> tagloader = new TagLoader<>(p_258247_ -> registry.getHolder(ResourceKey.create(resourcekey, p_258247_)), getTagDir(resourcekey));
