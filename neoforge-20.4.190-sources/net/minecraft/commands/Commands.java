...
public class Commands {
    ...
    public Commands(Commands.CommandSelection pSelection, CommandBuildContext pContext) {
        ...
        net.neoforged.neoforge.event.EventHooks.onCommandRegister(this.dispatcher, pSelection, pContext);
    ...
    public void performCommand(ParseResults<CommandSourceStack> pParseResults, String pCommand) {
        ...
        net.neoforged.neoforge.event.CommandEvent event = new net.neoforged.neoforge.event.CommandEvent(pParseResults);
        if (net.neoforged.neoforge.common.NeoForge.EVENT_BUS.post(event).isCanceled()) {
            if (event.getException() != null) {
                ...
                LOGGER.error("'/{}' threw an exception", pCommand, event.getException());
            }
            return;
        }
        pParseResults = event.getParseResults();
            ...
            if (contextchain != null) {
                executeCommandInContext(
                    ...
                    p_309417_ -> ExecutionContext.queueInitialCommandExecution(
                            p_309417_, pCommand, contextchain, commandsourcestack, CommandResultCallback.EMPTY
                        )
                );
            }
    ...
    public void sendCommands(ServerPlayer pPlayer) {
        ...
        net.neoforged.neoforge.server.command.CommandHelper.mergeCommandNode(this.dispatcher.getRoot(), rootcommandnode, map, pPlayer.createCommandSourceStack(), ctx -> 0, suggest -> SuggestionProviders.safelySwap((com.mojang.brigadier.suggestion.SuggestionProvider<SharedSuggestionProvider>) (com.mojang.brigadier.suggestion.SuggestionProvider<?>) suggest));
            ...
            public <T> HolderLookup<T> holderLookup(ResourceKey<? extends Registry<T>> p_256244_) {
                ...
                return new HolderLookup.Delegate<T>(registrylookup) {
                    ...
                    public HolderSet.Named<T> getOrThrow(TagKey<T> p_255953_) {
                        ...
                        return optional.orElseGet(() -> HolderSet.emptyNamed(registrylookup, p_255953_));
                    }
                };
            }
        };
    ...
    public static void validate() {
        ...
        Set<ArgumentType<?>> set1 = set.stream().filter(p_230961_ -> !ArgumentTypeInfos.isClassRecognized(p_230961_.getClass())).collect(Collectors.toSet());
        if (!set1.isEmpty()) {
            LOGGER.warn(
                "Missing type registration for following arguments:\n {}", set1.stream().map(p_230952_ -> "\t" + p_230952_).collect(Collectors.joining(",\n"))
            );
