...
public class ClientboundPlayerInfoUpdatePacket implements Packet<ClientGamePacketListener> {
    ...
    public static enum Action {
        ADD_PLAYER((p_251116_, p_251884_) -> {
            ...
            GameProfile gameprofile = Objects.requireNonNull(p_293733_.profile());
            p_293732_.writeUtf(gameprofile.getName(), 16);
            p_293732_.writeGameProfileProperties(gameprofile.getProperties());
        }),
        INITIALIZE_CHAT(
            (p_253468_, p_253469_) -> p_253468_.chatSession = p_253469_.readNullable(RemoteChatSession.Data::read),
            (p_253470_, p_253471_) -> p_253470_.writeNullable(p_253471_.chatSession, RemoteChatSession.Data::write)
        ),
        UPDATE_GAME_MODE(
            (p_251118_, p_248955_) -> p_251118_.gameMode = GameType.byId(p_248955_.readVarInt()),
            (p_249222_, p_250996_) -> p_249222_.writeVarInt(p_250996_.gameMode().getId())
        ),
        UPDATE_LISTED(
            (p_248777_, p_248837_) -> p_248777_.listed = p_248837_.readBoolean(), (p_293734_, p_293735_) -> p_293734_.writeBoolean(p_293735_.listed())
        ),
        UPDATE_LATENCY(
            (p_252263_, p_248964_) -> p_252263_.latency = p_248964_.readVarInt(), (p_248830_, p_251312_) -> p_248830_.writeVarInt(p_251312_.latency())
        ),
        UPDATE_DISPLAY_NAME(
            (p_248840_, p_251000_) -> p_248840_.displayName = p_251000_.readNullable(FriendlyByteBuf::readComponentTrusted),
            (p_251723_, p_251870_) -> p_251723_.writeNullable(p_251870_.displayName(), FriendlyByteBuf::writeComponent)
        );
