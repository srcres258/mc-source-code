...
public enum ConnectionProtocol {
    ...
    private ConnectionProtocol(String pId, ConnectionProtocol.ProtocolBuilder pBuilder) {
        this.id = pId;
        this.flows = pBuilder.buildCodecs(this);
    }

    @VisibleForDebug
    public Int2ObjectMap<Class<? extends Packet<?>>> getPacketsByIds(PacketFlow pPacketFlow) {
        return this.flows.get(pPacketFlow).packetsByIds();
    }
    ...
    public ConnectionProtocol.CodecData<?> codec(PacketFlow pFlow) {
        return this.flows.get(pFlow);
    }
    ...
    public static class CodecData<T extends PacketListener> implements BundlerInfo.Provider {
        ...
        public CodecData(ConnectionProtocol pProtocol, PacketFlow pFlow, ConnectionProtocol.PacketSet<T> pPacketSet) {
            this.protocol = pProtocol;
            this.flow = pFlow;
            this.packetSet = pPacketSet;
        }
        ...
        public int packetId(Packet<?> pPacket) {
            return this.packetSet.getId(pPacket.getClass());
        }
        ...
        /**
         * @deprecated Use {@link #createPacket(int, FriendlyByteBuf,
         *             io.netty.channel.ChannelHandlerContext)} instead, which provides
         *             the channel context for creating custom packet payloads.
         */
        @Nullable
        @Deprecated
        public Packet<?> createPacket(int pPacketId, FriendlyByteBuf pBuffer) {
            return this.packetSet.createPacket(pPacketId, pBuffer);
        }
        ...
        public boolean isValidPacketType(Packet<?> pPacket) {
            return this.packetSet.isKnownPacket(pPacket.getClass());
        }
    }
    ...
    static class PacketSet<T extends PacketListener> {
        ...
        public <P extends Packet<? super T>> ConnectionProtocol.PacketSet<T> addPacket(Class<P> pPacketClass, Function<FriendlyByteBuf, P> pDeserializer) {
            ...
            if (j != -1) {
                ...
                this.contextualIdToDeserializer.add((p_296217_, p_130535_) -> pDeserializer.apply(p_296217_)); //NeoForge: We always want to be able to create a packet from a buffer, even if we don't have a channel context
                return this;
            }
        }
        ...
        public <P extends BundlePacket<T>> ConnectionProtocol.PacketSet<T> withBundlePacket(Class<P> pPacketClass, Function<Iterable<Packet<? super T>>, P> pBundler) {
        ...
        public boolean isKnownPacket(Class<?> pPacketClass) {
            return this.classToId.containsKey(pPacketClass) || this.extraClasses.contains(pPacketClass);
        }
        ...
        @Nullable
        public Packet<?> createPacket(int pPacketId, FriendlyByteBuf pBuffer) {
            Function<FriendlyByteBuf, ? extends Packet<? super T>> function = this.idToDeserializer.get(pPacketId);
    ...
    static class ProtocolBuilder {
        ...
        public Map<PacketFlow, ConnectionProtocol.CodecData<?>> buildCodecs(ConnectionProtocol pProtocol) {
            ...
            for(PacketFlow packetflow : PacketFlow.values()) {
                ...
                if (packetset == null) {
                    throw new IllegalStateException("Missing packets for flow " + packetflow + " in protocol " + pProtocol);
                }

                map.put(packetflow, new ConnectionProtocol.CodecData<>(pProtocol, packetflow, packetset));
            }
