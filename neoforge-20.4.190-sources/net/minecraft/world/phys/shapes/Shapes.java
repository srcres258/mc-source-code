...
public final class Shapes {
        ...
        if (pFunction.apply(false, false)) {
            ...
            if (pShape1.isEmpty()) {
                ...
                IndexMerger indexmerger1 = createIndexMerger(
                    indexmerger.size() - 1, pShape1.getCoords(Direction.Axis.Y), pShape2.getCoords(Direction.Axis.Y), flag, flag1
                );
    ...
    public static VoxelShape getFaceShape(VoxelShape pVoxelShape, Direction pDirection) {
        if (pVoxelShape == block()) {
            ...
            if (pDirection.getAxisDirection() == Direction.AxisDirection.POSITIVE) {
                flag = DoubleMath.fuzzyEquals(pVoxelShape.max(direction$axis), 1.0, 1.0E-7);
                i = pVoxelShape.shape.getSize(direction$axis) - 1;
            } else {
                flag = DoubleMath.fuzzyEquals(pVoxelShape.min(direction$axis), 0.0, 1.0E-7);
                i = 0;
            }
    ...
    public static boolean faceShapeOccludes(VoxelShape pVoxelShape1, VoxelShape pVoxelShape2) {
        if (pVoxelShape1 == block() || pVoxelShape2 == block()) {
            return true;
        } else if (pVoxelShape1.isEmpty() && pVoxelShape2.isEmpty()) {
            return false;
        } else {
