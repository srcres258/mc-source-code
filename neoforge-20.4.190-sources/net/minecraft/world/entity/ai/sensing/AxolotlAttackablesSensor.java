...
public class AxolotlAttackablesSensor extends NearestVisibleLivingEntitySensor {
    ...
    protected boolean isMatchingEntity(LivingEntity pAttacker, LivingEntity pTarget) {
        return this.isClose(pAttacker, pTarget)
            && pTarget.isInWaterOrBubble()
            && (this.isHostileTarget(pTarget) || this.isHuntTarget(pAttacker, pTarget))
            && Sensor.isEntityAttackable(pAttacker, pTarget);
    }
