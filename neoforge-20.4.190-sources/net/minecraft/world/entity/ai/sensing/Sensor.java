...
public abstract class Sensor<E extends LivingEntity> {
    ...
    public static boolean isEntityTargetable(LivingEntity pLivingEntity, LivingEntity pTarget) {
        return pLivingEntity.getBrain().isMemoryValue(MemoryModuleType.ATTACK_TARGET, pTarget)
            ? TARGET_CONDITIONS_IGNORE_INVISIBILITY_TESTING.test(pLivingEntity, pTarget)
            : TARGET_CONDITIONS.test(pLivingEntity, pTarget);
    }
    ...
    public static boolean isEntityAttackable(LivingEntity pAttacker, LivingEntity pTarget) {
        return pAttacker.getBrain().isMemoryValue(MemoryModuleType.ATTACK_TARGET, pTarget)
            ? ATTACK_TARGET_CONDITIONS_IGNORE_INVISIBILITY_TESTING.test(pAttacker, pTarget)
            : ATTACK_TARGET_CONDITIONS.test(pAttacker, pTarget);
    }

    public static boolean isEntityAttackableIgnoringLineOfSight(LivingEntity pAttacker, LivingEntity pTarget) {
        return pAttacker.getBrain().isMemoryValue(MemoryModuleType.ATTACK_TARGET, pTarget)
            ? ATTACK_TARGET_CONDITIONS_IGNORE_INVISIBILITY_AND_LINE_OF_SIGHT.test(pAttacker, pTarget)
            : ATTACK_TARGET_CONDITIONS_IGNORE_LINE_OF_SIGHT.test(pAttacker, pTarget);
    }
}
