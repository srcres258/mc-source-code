...
public class Brain<E extends LivingEntity> {
            ...
            (new MapCodec<Brain<E>>() {
                    ...
                    public <T> Stream<T> keys(DynamicOps<T> p_22029_) {
                        return pMemoryTypes.stream()
                            .flatMap(p_22020_ -> p_22020_.getCodec().map(p_258254_ -> BuiltInRegistries.MEMORY_MODULE_TYPE.getKey(p_22020_)).stream())
                            ...
                            .forEach(
                                p_258252_ -> {
                                    DataResult<MemoryModuleType<?>> dataresult = BuiltInRegistries.MEMORY_MODULE_TYPE
                                        .byNameCodec()
                                        .parse(p_22022_, p_258252_.getFirst());
                                    DataResult<? extends Brain.MemoryValue<?>> dataresult1 = dataresult.flatMap(
                                        p_147350_ -> this.captureRead(p_147350_, p_22022_, (T)p_258252_.getSecond())
                                    );
    ...
    Stream<Brain.MemoryValue<?>> memories() {
        return this.memories
            .entrySet()
            .stream()
            .map(p_21929_ -> Brain.MemoryValue.createUnchecked((MemoryModuleType<? extends ExpirableValue<?>>)p_21929_.getKey(), p_21929_.getValue()));
    }
    ...
    public boolean checkMemory(MemoryModuleType<?> pMemoryType, MemoryStatus pMemoryStatus) {
        ...
        if (optional == null) {
            ...
            return pMemoryStatus == MemoryStatus.REGISTERED
    ...
    public void addActivityAndRemoveMemoryWhenStopped(
        Activity pActivity, int pPriorityStart, ImmutableList<? extends BehaviorControl<? super E>> pTasks, MemoryModuleType<?> pMemoryType
    ) {
                    ...
                    p_22053_ -> this.value
                            .ifPresent(
                                p_258258_ -> pBuilder.add(
                                        ...
                                        p_22053_.encodeStart(pOps, p_258258_)
                                    )
                            )
                );
        }
