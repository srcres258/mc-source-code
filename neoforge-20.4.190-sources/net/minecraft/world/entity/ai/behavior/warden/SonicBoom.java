...
public class SonicBoom extends Behavior<Warden> {
    ...
    protected void tick(ServerLevel pLevel, Warden pOwner, long pGameTime) {
        pOwner.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).ifPresent(p_311640_ -> pOwner.getLookControl().setLookAt(p_311640_.position()));
        if (!pOwner.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_DELAY)
            ...
            pOwner.getBrain()
                ...
                .ifPresent(p_217704_ -> {
                    Vec3 vec3 = pOwner.position().add(0.0, 1.6F, 0.0);
