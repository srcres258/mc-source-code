...
public class Sniffing<E extends Warden> extends Behavior<E> {
    ...
    protected void stop(ServerLevel pLevel, E pEntity, long pGameTime) {
        ...
        pEntity.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE).filter(pEntity::canTargetEntity).ifPresent(p_311638_ -> {
            if (pEntity.closerThan(p_311638_, 6.0, 20.0)) {
                pEntity.increaseAngerAt(p_311638_);
            }

            if (!pEntity.getBrain().hasMemoryValue(MemoryModuleType.DISTURBANCE_LOCATION)) {
                WardenAi.setDisturbanceLocation(pEntity, p_311638_.blockPosition());
            }
        });
    }
}
