...
public class GiveGiftToHero extends Behavior<Villager> {
    ...
    private List<ItemStack> getItemToThrow(Villager pVillager) {
        if (pVillager.isBaby()) {
            ...
            if (gift != null) {
                loottable = pVillager.level().getServer().getLootData().getLootTable(gift.lootTable());
            } else if (GIFTS.containsKey(villagerprofession)) {
                loottable = pVillager.level().getServer().getLootData().getLootTable(GIFTS.get(villagerprofession));
            }
            if (loottable != null) {
                LootParams lootparams = new LootParams.Builder((ServerLevel)pVillager.level())
                    .withParameter(LootContextParams.ORIGIN, pVillager.position())
                    .withParameter(LootContextParams.THIS_ENTITY, pVillager)
