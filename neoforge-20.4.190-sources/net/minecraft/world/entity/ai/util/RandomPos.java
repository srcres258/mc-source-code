...
public class RandomPos {
     ...
     *
     * @param pMaxHorizontalDifference The maximum value in x and z, in absolute value
     *                                 , that could be returned.
     * @param pYRange                  The range plus or minus the y position to be
     *                                 chosen
     * @param pY                       The target y position
     * @param pX                       The x offset to the target position
     * @param pZ                       The z offset to the target position
     * @param pMaxAngleDelta           The maximum variance of the returned angle,
     *                                 from the base angle being a vector from (0, 0)
     *                                 to (x, z).
     */
    @Nullable
    public static BlockPos generateRandomDirectionWithinRadians(
        RandomSource pRandom, int pMaxHorizontalDifference, int pYRange, int pY, double pX, double pZ, double pMaxAngleDelta
    ) {
    ...
    public static BlockPos moveUpOutOfSolid(BlockPos pPos, int pMaxY, Predicate<BlockPos> pPosPredicate) {
        if (!pPosPredicate.test(pPos)) {
            ...
            while(blockpos.getY() < pMaxY && pPosPredicate.test(blockpos)) {
