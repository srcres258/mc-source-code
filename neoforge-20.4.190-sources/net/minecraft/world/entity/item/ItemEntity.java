...
public class ItemEntity extends Entity implements TraceableEntity {
    ...
    public static boolean areMergable(ItemStack pDestinationStack, ItemStack pOriginStack) {
        if (!pOriginStack.is(pDestinationStack.getItem())) {
            ...
            return false;
        } else if (!pDestinationStack.areAttachmentsCompatible(pOriginStack)) {
            return false;
        } else {
    ...
    public void playerTouch(Player pEntity) {
        if (!this.level().isClientSide) {
            ...
            int hook = net.neoforged.neoforge.event.EventHooks.onItemPickup(this, pEntity);
            if (hook < 0) return;
            ...
            if (this.pickupDelay == 0 && (this.target == null || this.target.equals(pEntity.getUUID())) && (hook == 1 || i <= 0 || pEntity.getInventory().add(itemstack))) {
                ...
                copy.setCount(i);
                net.neoforged.neoforge.event.EventHooks.firePlayerItemPickupEvent(pEntity, this, copy);
                pEntity.take(this, i);
