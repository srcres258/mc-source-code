...
public class Pig extends Animal implements ItemSteerable, Saddleable {
    ...
    public LivingEntity getControllingPassenger() {
        if (this.isSaddled()) {
            ...
            if (entity instanceof Player player && player.isHolding(Items.CARROT_ON_A_STICK)) {
                return player;
            }
        }
    ...
    public void thunderHit(ServerLevel pLevel, LightningBolt pLightning) {
        if (pLevel.getDifficulty() != Difficulty.PEACEFUL && net.neoforged.neoforge.event.EventHooks.canLivingConvert(this, EntityType.ZOMBIFIED_PIGLIN, (timer) -> {})) {
    ...
    protected Vector3f getPassengerAttachmentPoint(Entity pEntity, EntityDimensions pDimensions, float pScale) {
        return new Vector3f(0.0F, pDimensions.height - 0.03125F * pScale, 0.0F);
    }
}
