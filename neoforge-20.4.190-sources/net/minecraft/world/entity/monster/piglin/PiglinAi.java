...
public class PiglinAi {
    ...
    private static void initFightActivity(Piglin pPiglin, Brain<Piglin> pBrain) {
        pBrain.addActivityAndRemoveMemoryWhenStopped(
            ...
            ImmutableList.<net.minecraft.world.entity.ai.behavior.BehaviorControl<? super Piglin>>of(
                StopAttackingIfTargetInvalid.<Piglin>create(p_34981_ -> !isNearestValidAttackTarget(pPiglin, p_34981_)),
    ...
    private static List<ItemStack> getBarterResponseItems(Piglin pPiglin) {
        ...
        List<ItemStack> list = loottable.getRandomItems(
            new LootParams.Builder((ServerLevel)pPiglin.level())
                .withParameter(LootContextParams.THIS_ENTITY, pPiglin)
    ...
    public static void angerNearbyPiglins(Player pPlayer, boolean pAngerOnlyIfCanSee) {
        ...
        list.stream().filter(PiglinAi::isIdle).filter(p_34881_ -> !pAngerOnlyIfCanSee || BehaviorUtils.canSee(p_34881_, pPlayer)).forEach(p_311688_ -> {
            if (p_311688_.level().getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                setAngerTargetToNearestTargetablePlayerIfFound(p_311688_, pPlayer);
            } else {
                setAngerTarget(p_311688_, pPlayer);
            }
        });
    }
    ...
    protected static void wasHurtBy(Piglin pPiglin, LivingEntity pTarget) {
        if (!(pTarget instanceof Piglin)) {
            ...
            getAvoidTarget(pPiglin).ifPresent(p_308764_ -> {
                if (p_308764_.getType() != pTarget.getType()) {
    ...
    private static BehaviorControl<LivingEntity> babySometimesRideBabyHoglin() {
        ...
        return CopyMemoryWithExpiry.create(
            p_311686_ -> p_311686_.isBaby() && setentitylooktargetsometimes$ticker.tickDownAndCheck(p_311686_.level().random),
    ...
    protected static void broadcastAngerTarget(AbstractPiglin pPiglin, LivingEntity pTarget) {
        getAdultPiglins(pPiglin).forEach(p_308761_ -> {
            if (pTarget.getType() != EntityType.HOGLIN || p_308761_.canHunt() && ((Hoglin)pTarget).canBeHunted()) {
                setAngerTargetIfCloserThanCurrent(p_308761_, pTarget);
            }
        });
    }
    ...
    public static Optional<LivingEntity> getAvoidTarget(Piglin pPiglin) {
        return pPiglin.getBrain().hasMemoryValue(MemoryModuleType.AVOID_TARGET)
            ? pPiglin.getBrain().getMemory(MemoryModuleType.AVOID_TARGET)
            : Optional.empty();
    }

    public static Optional<Player> getNearestVisibleTargetablePlayer(AbstractPiglin pPiglin) {
        return pPiglin.getBrain().hasMemoryValue(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYER)
            ? pPiglin.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYER)
            : Optional.empty();
    }

    private static void broadcastRetreat(Piglin pPiglin, LivingEntity pTarget) {
        getVisibleAdultPiglins(pPiglin)
            .stream()
            .filter(p_34985_ -> p_34985_ instanceof Piglin)
            .forEach(p_34819_ -> retreatFromNearestTarget((Piglin)p_34819_, pTarget));
    }
