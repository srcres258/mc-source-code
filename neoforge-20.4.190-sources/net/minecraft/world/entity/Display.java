...
public abstract class Display extends Entity {
    ...
    public void onSyncedDataUpdated(EntityDataAccessor<?> pKey) {
        ...
        if (DATA_TRANSFORMATION_INTERPOLATION_START_DELTA_TICKS_ID.equals(pKey)) {
            this.updateStartTick = true;
        }

        if (DATA_TRANSFORMATION_INTERPOLATION_DURATION_ID.equals(pKey)) {
    ...
    protected void readAdditionalSaveData(CompoundTag pCompound) {
        if (pCompound.contains("transformation")) {
            Transformation.EXTENDED_CODEC
                .decode(NbtOps.INSTANCE, pCompound.get("transformation"))
        ...
        if (pCompound.contains("teleport_duration", 99)) {
            int k = pCompound.getInt("teleport_duration");
            this.setPosRotInterpolationDuration(Mth.clamp(k, 0, 59));
        }

        if (pCompound.contains("billboard", 8)) {
            Display.BillboardConstraints.CODEC
                .decode(NbtOps.INSTANCE, pCompound.get("billboard"))
                .resultOrPartial(Util.prefix("Display entity", LOGGER::error))
        ...
        if (pCompound.contains("brightness", 10)) {
            Brightness.CODEC
                .decode(NbtOps.INSTANCE, pCompound.get("brightness"))
                .resultOrPartial(Util.prefix("Display entity", LOGGER::error))
    ...
    protected void addAdditionalSaveData(CompoundTag pCompound) {
        Transformation.EXTENDED_CODEC
            ...
            .result()
            .ifPresent(p_270528_ -> pCompound.put("transformation", p_270528_));
        Display.BillboardConstraints.CODEC
            .encodeStart(NbtOps.INSTANCE, this.getBillboardConstraints())
            .result()
            .ifPresent(p_270227_ -> pCompound.put("billboard", p_270227_));
        pCompound.putInt("interpolation_duration", this.getTransformationInterpolationDuration());
        pCompound.putInt("teleport_duration", this.getPosRotInterpolationDuration());
        ...
        if (brightness != null) {
            Brightness.CODEC.encodeStart(NbtOps.INSTANCE, brightness).result().ifPresent(p_270121_ -> pCompound.put("brightness", p_270121_));
        }
    }
    ...
    public void lerpTo(double pX, double pY, double pZ, float pYRot, float pXRot, int pSteps) {
        ...
        this.posRotInterpolationTarget = new Display.PosRotInterpolationTarget(i, pX, pY, pZ, (double)pYRot, (double)pXRot);
    }
    ...
    public boolean shouldRenderAtSqrDistance(double pDistance) {
        return pDistance < Mth.square((double)this.getViewRange() * 64.0 * getViewScale());
    }
    ...
    public static class ItemDisplay extends Display {
        ...
        protected void readAdditionalSaveData(CompoundTag pCompound) {
            ...
            if (pCompound.contains("item_display", 8)) {
                ItemDisplayContext.CODEC
                    .decode(NbtOps.INSTANCE, pCompound.get("item_display"))
        ...
        protected void addAdditionalSaveData(CompoundTag pCompound) {
            ...
            ItemDisplayContext.CODEC
                .encodeStart(NbtOps.INSTANCE, this.getItemTransform())
                .result()
                .ifPresent(p_270615_ -> pCompound.put("item_display", p_270615_));
        }
    ...
    public static class TextDisplay extends Display {
        ...
        protected void readAdditionalSaveData(CompoundTag pCompound) {
            ...
            Optional<Display.TextDisplay.Align> optional = Display.TextDisplay.Align.CODEC
                .decode(NbtOps.INSTANCE, pCompound.get("alignment"))
                .resultOrPartial(Util.prefix("Display entity", Display.LOGGER::error))
                .map(Pair::getFirst);
        ...
        protected void addAdditionalSaveData(CompoundTag pCompound) {
            ...
            Display.TextDisplay.Align.CODEC.encodeStart(NbtOps.INSTANCE, getAlign(b0)).result().ifPresent(p_271001_ -> pCompound.put("alignment", p_271001_));
        }
