...
public record VibrationInfo(GameEvent gameEvent, float distance, Vec3 pos, @Nullable UUID uuid, @Nullable UUID projectileOwnerUuid, @Nullable Entity entity) {
    public static final Codec<VibrationInfo> CODEC = RecordCodecBuilder.create(
        p_258994_ -> p_258994_.group(
                    ...
                    UUIDUtil.CODEC.optionalFieldOf("source").forGetter(p_250608_ -> Optional.ofNullable(p_250608_.uuid())),
                    UUIDUtil.CODEC.optionalFieldOf("projectile_owner").forGetter(p_250607_ -> Optional.ofNullable(p_250607_.projectileOwnerUuid()))
                )
                .apply(
                    p_258994_,
                    (p_249268_, p_252231_, p_250951_, p_250574_, p_249661_) -> new VibrationInfo(
                            p_249268_, p_252231_, p_250951_, p_250574_.orElse(null), p_249661_.orElse(null)
                        )
                )
    );
