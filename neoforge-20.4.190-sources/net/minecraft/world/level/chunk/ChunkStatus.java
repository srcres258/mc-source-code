...
public class ChunkStatus {
    ...
    private static final ChunkStatus.LoadingTask PASSTHROUGH_LOAD_TASK = (p_281194_, p_281195_, p_281196_, p_281197_, p_281198_, p_281199_) -> CompletableFuture.completedFuture(
            Either.left(p_281199_)
        );
    public static final ChunkStatus EMPTY = registerSimple(
        "empty", null, -1, PRE_FEATURES, ChunkStatus.ChunkType.PROTOCHUNK, (p_156307_, p_156308_, p_156309_, p_156310_, p_156311_) -> {
        }
    );
    public static final ChunkStatus STRUCTURE_STARTS = register(
        ...
        (p_309516_, p_309517_, p_309518_, p_309519_, p_309520_, p_309521_, p_309522_, p_309523_, p_309524_) -> {
            if (p_309518_.getServer().getWorldData().worldGenOptions().generateStructures()) {
                p_309519_.createStructures(
                    p_309518_.registryAccess(), p_309518_.getChunkSource().getGeneratorState(), p_309518_.structureManager(), p_309524_, p_309520_
                );
            }
    
            p_309518_.onStructureStartsAvailable(p_309524_);
            return CompletableFuture.completedFuture(Either.left(p_309524_));
        },
    ...
    public static final ChunkStatus BIOMES = register(
        ...
        PRE_FEATURES,
        ChunkStatus.ChunkType.PROTOCHUNK,
        (p_281200_, p_281201_, p_281202_, p_281203_, p_281204_, p_281205_, p_281206_, p_281207_, p_281208_) -> {
            WorldGenRegion worldgenregion = new WorldGenRegion(p_281202_, p_281207_, p_281200_, -1);
            return p_281203_.createBiomes(
                    p_281201_,
                    p_281202_.getChunkSource().randomState(),
                    Blender.of(worldgenregion),
                    p_281202_.structureManager().forWorldGenRegion(worldgenregion),
                    p_281208_
                )
                .thenApply(p_281193_ -> Either.left(p_281193_));
        }
    );
    public static final ChunkStatus NOISE = register(
        ...
        PRE_FEATURES,
        ChunkStatus.ChunkType.PROTOCHUNK,
        (p_281161_, p_281162_, p_281163_, p_281164_, p_281165_, p_281166_, p_281167_, p_281168_, p_281169_) -> {
            WorldGenRegion worldgenregion = new WorldGenRegion(p_281163_, p_281168_, p_281161_, 0);
            return p_281164_.fillFromNoise(
                    p_281162_,
                    Blender.of(worldgenregion),
                    p_281163_.getChunkSource().randomState(),
                    p_281163_.structureManager().forWorldGenRegion(worldgenregion),
                    p_281169_
                )
                .thenApply(p_281218_ -> {
    ...
    public static final ChunkStatus SURFACE = registerSimple(
        ...
        (p_156247_, p_156248_, p_156249_, p_156250_, p_156251_) -> {
            ...
            p_156249_.buildSurface(
                worldgenregion, p_156248_.structureManager().forWorldGenRegion(worldgenregion), p_156248_.getChunkSource().randomState(), p_156251_
            );
        }
    );
    public static final ChunkStatus CARVERS = registerSimple(
        ...
        POST_FEATURES,
        ChunkStatus.ChunkType.PROTOCHUNK,
        (p_309525_, p_309526_, p_309527_, p_309528_, p_309529_) -> {
            WorldGenRegion worldgenregion = new WorldGenRegion(p_309526_, p_309528_, p_309525_, 0);
            if (p_309529_ instanceof ProtoChunk protochunk) {
                Blender.addAroundOldChunksCarvingMaskFilter(worldgenregion, protochunk);
            }
    
            p_309527_.applyCarvers(
                worldgenregion,
                p_309526_.getSeed(),
                p_309526_.getChunkSource().randomState(),
                p_309526_.getBiomeManager(),
                p_309526_.structureManager().forWorldGenRegion(worldgenregion),
                p_309529_,
    ...
    public static final ChunkStatus FEATURES = registerSimple(
        ...
        POST_FEATURES,
        ChunkStatus.ChunkType.PROTOCHUNK,
        (p_281188_, p_281189_, p_281190_, p_281191_, p_281192_) -> {
    ...
    public static final ChunkStatus INITIALIZE_LIGHT = register(
        ...
        POST_FEATURES,
        ChunkStatus.ChunkType.PROTOCHUNK,
        (p_281179_, p_281180_, p_281181_, p_281182_, p_281183_, p_281184_, p_281185_, p_281186_, p_281187_) -> initializeLight(p_281184_, p_281187_),
        (p_281155_, p_281156_, p_281157_, p_281158_, p_281159_, p_281160_) -> initializeLight(p_281158_, p_281160_)
    );
    public static final ChunkStatus LIGHT = register(
        ...
        POST_FEATURES,
        ChunkStatus.ChunkType.PROTOCHUNK,
        (p_284904_, p_284905_, p_284906_, p_284907_, p_284908_, p_284909_, p_284910_, p_284911_, p_284912_) -> lightChunk(p_284909_, p_284912_),
        (p_284898_, p_284899_, p_284900_, p_284901_, p_284902_, p_284903_) -> lightChunk(p_284901_, p_284903_)
    );
    public static final ChunkStatus SPAWN = registerSimple(
        "spawn", LIGHT, 0, POST_FEATURES, ChunkStatus.ChunkType.PROTOCHUNK, (p_196758_, p_196759_, p_196760_, p_196761_, p_196762_) -> {
    ...
    public static final ChunkStatus FULL = register(
        ...
        (p_223267_, p_223268_, p_223269_, p_223270_, p_223271_, p_223272_, p_223273_, p_223274_, p_223275_) -> p_223273_.apply(p_223275_),
        (p_223260_, p_223261_, p_223262_, p_223263_, p_223264_, p_223265_) -> p_223264_.apply(p_223265_)
    );
    ...
    private static CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> initializeLight(
        ThreadedLevelLightEngine pLightEngine, ChunkAccess pChunk
    ) {
    ...
    private static ChunkStatus registerSimple(
        ...
        int pTaskRange,
        EnumSet<Heightmap.Types> pHeightmaps,
        ChunkStatus.ChunkType pType,
        ChunkStatus.SimpleGenerationTask pGenerationTask
    ) {
    ...
    private static ChunkStatus register(
        ...
        return Registry.register(
            BuiltInRegistries.CHUNK_STATUS, pKey, new ChunkStatus(pParent, pTaskRange, pHasLoadDependencies, pHeightmaps, pType, pGenerationTask, pLoadingTask)
        );
    }
    ...
    public ChunkStatus(
        ...
        ChunkStatus.ChunkType pChunkType,
        ChunkStatus.GenerationTask pGenerationTask,
        ChunkStatus.LoadingTask pLoadingTask
    ) {
    ...
    public CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> generate(
        ...
        return this.generationTask
            .doWork(this, pExectutor, pLevel, pChunkGenerator, pStructureTemplateManager, pLightEngine, pTask, pCache, chunkaccess)
            .thenApply(
                (Function<? super Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>, ? extends Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>>)(p_281217_ -> {
                    p_281217_.ifLeft(p_290029_ -> {
                        if (p_290029_ instanceof ProtoChunk protochunk && !protochunk.getStatus().isOrAfter(this)) {
