...
public abstract class ChunkGenerator {
                        ...
                        for(Structure structure : map.getOrDefault(k, Collections.emptyList())) {
                            ...
                            try {
                                ...
                                pStructureManager.startsForStructure(sectionpos, structure)
                                    .forEach(
                                        p_223086_ -> p_223086_.placeInChunk(pLevel, pStructureManager, this, worldgenrandom, getWritableArea(pChunk), chunkpos)
                                    );
            ...
            .forEach(
                p_255564_ -> {
                    ...
                    for(StructureSet.StructureSelectionEntry structureset$structureselectionentry : list) {
                        StructureStart structurestart = pStructureManager.getStartForStructure(
                            sectionpos, structureset$structureselectionentry.structure().value(), pChunk
                        );
                    ...
                    if (structureplacement.isStructureChunk(pStructureState, chunkpos.x, chunkpos.z)) {
                        if (list.size() == 1) {
                            this.tryGenerateStructure(
                                list.get(0), pStructureManager, pRegistryAccess, randomstate, pStructureTemplateManager, pStructureState.getLevelSeed(), pChunk, chunkpos, sectionpos
                            );
                        } else {
    ...
    private boolean tryGenerateStructure(
        ...
        StructureStart structurestart = structure.generate(
            pRegistryAccess, this, this.biomeSource, pRandom, pStructureTemplateManager, pSeed, pChunkPos, i, pChunk, predicate
        );
