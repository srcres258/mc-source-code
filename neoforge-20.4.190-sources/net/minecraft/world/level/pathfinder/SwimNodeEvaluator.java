...
public class SwimNodeEvaluator extends NodeEvaluator {
    ...
    public int getNeighbors(Node[] pOutputArray, Node pNode) {
        ...
        for(Direction direction1 : Direction.Plane.HORIZONTAL) {
            ...
            Node node1 = this.findAcceptedNode(
                pNode.x + direction1.getStepX() + direction2.getStepX(), pNode.y, pNode.z + direction1.getStepZ() + direction2.getStepZ()
            );
    ...
    protected BlockPathTypes getCachedBlockType(int pX, int pY, int pZ) {
        return this.pathTypesByPosCache
            .computeIfAbsent(BlockPos.asLong(pX, pY, pZ), p_192957_ -> this.getBlockPathType(this.level, pX, pY, pZ));
    }
            ...
            for(int j = pY; j < pY + this.entityHeight; ++j) {
                for(int k = pZ; k < pZ + this.entityDepth; ++k) {
                    ...
                    if (fluidstate.isEmpty()
                        && blockstate.isPathfindable(pLevel, blockpos$mutableblockpos.below(), PathComputationType.WATER)
                        && blockstate.isAir()) {
