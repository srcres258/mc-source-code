...
public class MapItemSavedData extends SavedData {
    ...
    public CompoundTag save(CompoundTag pCompound) {
        ResourceLocation.CODEC
            ...
            .resultOrPartial(LOGGER::error)
            .ifPresent(p_77954_ -> pCompound.put("dimension", p_77954_));
    ...
    public MapItemSavedData scaled(int pScalar) {
        return createFresh(
            (double)this.centerX,
            (double)this.centerZ,
            (byte)Mth.clamp(this.scale + pScalar, 0, 4),
    ...
    public void tickCarriedBy(Player pPlayer, ItemStack pMapStack) {
        ...
        for(int i = 0; i < this.carriedBy.size(); ++i) {
            ...
            if (!mapitemsaveddata$holdingplayer1.player.isRemoved()
                && (mapitemsaveddata$holdingplayer1.player.getInventory().contains(pMapStack) || pMapStack.isFramed())) {
        ...
        if (compoundtag != null && compoundtag.contains("Decorations", 9)) {
            ...
            for(int j = 0; j < listtag.size(); ++j) {
                ...
                if (!this.decorations.containsKey(compoundtag1.getString("id"))) {
                    this.addDecoration(
                        MapDecoration.Type.byIcon(compoundtag1.getByte("type")),
                        pPlayer.level(),
                        compoundtag1.getString("id"),
                        compoundtag1.getDouble("x"),
                        compoundtag1.getDouble("z"),
                        compoundtag1.getDouble("rot"),
                        null
                    );
                }
            }
    ...
    private void addDecoration(
        MapDecoration.Type pType,
        @Nullable LevelAccessor pLevel,
        String pDecorationName,
        double pLevelX,
        double pLevelZ,
        double pRotation,
        @Nullable Component pName
    ) {
        int i = 1 << this.scale;
        ...
        if (f >= -63.0F && f1 >= -63.0F && f <= 63.0F && f1 <= 63.0F) {
            pRotation += pRotation < 0.0 ? -8.0 : 8.0;
            b2 = (byte)((int)(pRotation * 16.0 / 360.0));
    ...
    public boolean toggleBanner(LevelAccessor pAccessor, BlockPos pPos) {
        ...
        if (d2 >= -63.0 && d3 >= -63.0 && d2 <= 63.0 && d3 <= 63.0) {
            ...
            if (!this.isTrackedCountOverLimit(256)) {
                ...
                this.addDecoration(mapbanner.getDecoration(), pAccessor, mapbanner.getId(), d0, d1, 180.0, mapbanner.getName());
                return true;
            }
        }
    ...
    public class HoldingPlayer {
        ...
        Packet<?> nextUpdatePacket(int pMapId) {
            ...
            return collection == null && mapitemsaveddata$mappatch == null
                ? null
                : new ClientboundMapItemDataPacket(pMapId, MapItemSavedData.this.scale, MapItemSavedData.this.locked, collection, mapitemsaveddata$mappatch);
        }
