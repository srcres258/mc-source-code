...
public class BellBlockEntity extends BlockEntity {
    ...
    private static void showBellParticles(Level p_155208_, BlockPos p_155209_, List<LivingEntity> p_155210_) {
        ...
        int i = (int)p_155210_.stream().filter(p_311749_ -> p_155209_.closerToCenterThan(p_311749_.position(), 48.0)).count();
        p_155210_.stream()
            ...
            .forEach(
                p_155195_ -> {
                    float f = 1.0F;
                    double d0 = Math.sqrt(
                        (p_155195_.getX() - (double)p_155209_.getX()) * (p_155195_.getX() - (double)p_155209_.getX())
                            + (p_155195_.getZ() - (double)p_155209_.getZ()) * (p_155195_.getZ() - (double)p_155209_.getZ())
                    );
                    double d1 = (double)((float)p_155209_.getX() + 0.5F) + 1.0 / d0 * (p_155195_.getX() - (double)p_155209_.getX());
                    double d2 = (double)((float)p_155209_.getZ() + 0.5F) + 1.0 / d0 * (p_155195_.getZ() - (double)p_155209_.getZ());
                    int j = Mth.clamp((i - 21) / -2, 3, 15);
        
                    for(int k = 0; k < j; ++k) {
                        int l = mutableint.addAndGet(5);
                        double d3 = (double)FastColor.ARGB32.red(l) / 255.0;
                        double d4 = (double)FastColor.ARGB32.green(l) / 255.0;
                        double d5 = (double)FastColor.ARGB32.blue(l) / 255.0;
