...
public class HopperBlockEntity extends RandomizableContainerBlockEntity implements Hopper {
    ...
    private static IntStream getSlots(Container pContainer, Direction pDirection) {
        return pContainer instanceof WorldlyContainer
            ? IntStream.of(((WorldlyContainer)pContainer).getSlotsForFace(pDirection))
            : IntStream.range(0, pContainer.getContainerSize());
    }
    ...
    public static boolean suckInItems(Level pLevel, Hopper pHopper) {
        ...
        if (container != null) {
            ...
            return isEmptyContainer(container, direction)
                ? false
                : getSlots(container, direction).anyMatch(p_59363_ -> tryTakeInItemFromSlot(pHopper, container, p_59363_, direction));
        } else {
    ...
    private static boolean tryTakeInItemFromSlot(Hopper pHopper, Container pContainer, int pSlot, Direction pDirection) {
        ...
        if (!itemstack.isEmpty() && canTakeItemFromContainer(pHopper, pContainer, itemstack, pSlot, pDirection)) {
            ...
            ItemStack itemstack2 = addItem(pContainer, pHopper, pContainer.removeItem(pSlot, 1), null);
    ...
    private static Container getSourceContainer(Level pLevel, Hopper pHopper) {
        return getContainerAt(pLevel, pHopper.getLevelX(), pHopper.getLevelY() + 1.0, pHopper.getLevelZ());
    }

    public static List<ItemEntity> getItemsAtAndAbove(Level pLevel, Hopper pHopper) {
        return pHopper.getSuckShape()
            .toAabbs()
            .stream()
            .flatMap(
                p_155558_ -> pLevel.getEntitiesOfClass(
                            ItemEntity.class,
                            p_155558_.move(pHopper.getLevelX() - 0.5, pHopper.getLevelY() - 0.5, pHopper.getLevelZ() - 0.5),
    ...
    public static Container getContainerAt(Level pLevel, BlockPos pPos) {
        return getContainerAt(pLevel, (double)pPos.getX() + 0.5, (double)pPos.getY() + 0.5, (double)pPos.getZ() + 0.5);
    }
