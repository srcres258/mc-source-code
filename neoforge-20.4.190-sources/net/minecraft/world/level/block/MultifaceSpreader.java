...
public class MultifaceSpreader {
    ...
    public boolean canSpreadInAnyDirection(BlockState pState, BlockGetter pLevel, BlockPos pPos, Direction pSpreadDirection) {
        return Direction.stream()
            .anyMatch(
                p_221611_ -> this.getSpreadFromFaceTowardDirection(pState, pLevel, pPos, pSpreadDirection, p_221611_, this.config::canSpreadInto)
                        .isPresent()
            );
    }

    public Optional<MultifaceSpreader.SpreadPos> spreadFromRandomFaceTowardRandomDirection(
        BlockState pState, LevelAccessor pLevel, BlockPos pPos, RandomSource pRandom
    ) {
