...
public class IcebergFeature extends Feature<BlockStateConfiguration> {
     ...
     * @param pAngle       The rotation angle of the ellipse (the angle from the
    ...
    private double signedDistanceEllipse(int pX, int pZ, BlockPos pCenter, int pMajorRadius, int pMinorRadius, double pAngle) {
        return Math.pow(
                ((double)(pX - pCenter.getX()) * Math.cos(pAngle) - (double)(pZ - pCenter.getZ()) * Math.sin(pAngle)) / (double)pMajorRadius, 2.0
            )
            + Math.pow(
                ((double)(pX - pCenter.getX()) * Math.sin(pAngle) + (double)(pZ - pCenter.getZ()) * Math.cos(pAngle)) / (double)pMinorRadius, 2.0
            )
            - 1.0;
    }
