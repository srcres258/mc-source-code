...
public class FancyTrunkPlacer extends TrunkPlacer {
    public static final Codec<FancyTrunkPlacer> CODEC = RecordCodecBuilder.create(p_70136_ -> trunkPlacerParts(p_70136_).apply(p_70136_, FancyTrunkPlacer::new));
        ...
        for(; j1 >= 0; --j1) {
            ...
            if (!(f < 0.0F)) {
                for(int k1 = 0; k1 < l; ++k1) {
                    double d1 = 1.0;
                    double d2 = 1.0 * (double)f * ((double)pRandom.nextFloat() + 0.328);
            ...
            for(int j = 0; j <= i; ++j) {
                ...
                if (pModifyWorld) {
                    this.placeLog(
                        ...
                        blockpos1,
                        pConfig,
                        p_161826_ -> p_161826_.trySetValue(RotatedPillarBlock.AXIS, this.getLogAxis(pBasePos, blockpos1))
                    );
