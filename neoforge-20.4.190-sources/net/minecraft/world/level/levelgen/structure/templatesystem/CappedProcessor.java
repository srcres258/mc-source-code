            ...
            if (pOriginalBlockInfos.size() != pProcessedBlockInfos.size()) {
                Util.logAndPauseIfInIde(
                    "Original block info list not in sync with processed list, skipping processing. Original size: "
                        + pOriginalBlockInfos.size()
                        + ", Processed size: "
                        + pProcessedBlockInfos.size()
                );
                ...
                if (i < 1) {
                    ...
                    while(intiterator.hasNext() && j < i) {
                        ...
                        StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo2 = this.delegate
                            .processBlock(
                                pServerLevel, pOffset, pPos, structuretemplate$structureblockinfo, structuretemplate$structureblockinfo1, pSettings
                            );
