...
public class LegacyStructureDataHandler {
    ...
    private boolean hasLegacyStart(int pChunkX, int pChunkZ, String pKey) {
        if (!this.hasLegacyData) {
            ...
            return this.dataMap.get(pKey) != null
                && this.indexMap.get(CURRENT_TO_LEGACY_MAP.get(pKey)).hasStartIndex(ChunkPos.asLong(pChunkX, pChunkZ));
        }
    }
    ...
    private void populateCaches(@Nullable DimensionDataStorage pStorage) {
        if (pStorage != null) {
            for(String s : this.legacyKeys) {
                ...
                try {
                    compoundtag = pStorage.readTagFromDisk(s, DataFixTypes.SAVED_DATA_STRUCTURE_FEATURE_INDICES, 1493)
                        .getCompound("data")
