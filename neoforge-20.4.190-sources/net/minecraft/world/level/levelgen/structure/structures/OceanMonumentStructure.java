...
public class OceanMonumentStructure extends Structure {
    ...
    public Optional<Structure.GenerationStub> findGenerationPoint(Structure.GenerationContext pContext) {
        ...
        for(Holder<Biome> holder : pContext.biomeSource()
            .getBiomesWithin(i, pContext.chunkGenerator().getSeaLevel(), j, 29, pContext.randomState().sampler())) {
            if (!holder.is(BiomeTags.REQUIRED_OCEAN_MONUMENT_SURROUNDING)) {
                return Optional.empty();
            }
        }

        return onTopOfChunkCenter(pContext, Heightmap.Types.OCEAN_FLOOR_WG, p_228967_ -> generatePieces(p_228967_, pContext));
    }
