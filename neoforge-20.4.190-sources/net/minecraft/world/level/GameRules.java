...
public class GameRules {
    ...
    public GameRules() {
        this.rules = GAME_RULE_TYPES.entrySet()
            .stream()
            .collect(ImmutableMap.toImmutableMap(Entry::getKey, p_46210_ -> ((GameRules.Type)p_46210_.getValue()).createRule()));
    }
    ...
    public GameRules copy() {
        return new GameRules(
            this.rules.entrySet().stream().collect(ImmutableMap.toImmutableMap(Entry::getKey, p_46194_ -> ((GameRules.Value)p_46194_.getValue()).copy()))
        );
    }

    public static void visitGameRuleTypes(GameRules.GameRuleTypeVisitor pVisitor) {
        GAME_RULE_TYPES.forEach((p_46205_, p_46206_) -> callVisitorCap(pVisitor, p_46205_, p_46206_));
    }

    private static <T extends GameRules.Value<T>> void callVisitorCap(
        GameRules.GameRuleTypeVisitor pVisitor, GameRules.Key<?> pKey, GameRules.Type<?> pType
    ) {
    ...
    public void assignFrom(GameRules pRules, @Nullable MinecraftServer pServer) {
        pRules.rules.keySet().forEach(p_46182_ -> this.assignCap(p_46182_, pRules, pServer));
    }
    ...
    public static class BooleanValue extends GameRules.Value<GameRules.BooleanValue> {
        ...
        public static GameRules.Type<GameRules.BooleanValue> create(boolean pDefaultValue, BiConsumer<MinecraftServer, GameRules.BooleanValue> pChangeListener) {
            return new GameRules.Type<>(
                BoolArgumentType::bool, p_46242_ -> new GameRules.BooleanValue(p_46242_, pDefaultValue), pChangeListener, GameRules.GameRuleTypeVisitor::visitBoolean
            );
        }
    ...
    public static class IntegerValue extends GameRules.Value<GameRules.IntegerValue> {
        ...
        public static GameRules.Type<GameRules.IntegerValue> create(int pDefaultValue, BiConsumer<MinecraftServer, GameRules.IntegerValue> pChangeListener) {
            return new GameRules.Type<>(
                IntegerArgumentType::integer, p_46293_ -> new GameRules.IntegerValue(p_46293_, pDefaultValue), pChangeListener, GameRules.GameRuleTypeVisitor::visitInteger
            );
        }
    ...
    public static class Type<T extends GameRules.Value<T>> {
        ...
        public RequiredArgumentBuilder<CommandSourceStack, ?> createArgument(String pName) {
            return Commands.argument(pName, (ArgumentType<T>)this.argument.get());
        }
