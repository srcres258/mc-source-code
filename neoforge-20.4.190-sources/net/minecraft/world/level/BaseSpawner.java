...
public abstract class BaseSpawner {
    ...
    public void setEntityId(EntityType<?> pType, @Nullable Level pLevel, RandomSource pRandom, BlockPos pPos) {
        this.getOrCreateNextSpawnData(pLevel, pRandom, pPos)
            .getEntityToSpawn()
            .putString("id", BuiltInRegistries.ENTITY_TYPE.getKey(pType).toString());
    }
                ...
                for(int i = 0; i < this.spawnCount; ++i) {
                    ...
                    if (pServerLevel.noCollision(optional.get().getAABB(d0, d1, d2))) {
                        ...
                        if (spawndata.getCustomSpawnRules().isPresent()) {
                            ...
                            if (!spawndata$customspawnrules.blockLightLimit().isValueInRange(pServerLevel.getBrightness(LightLayer.BLOCK, blockpos))
                                || !spawndata$customspawnrules.skyLightLimit().isValueInRange(pServerLevel.getBrightness(LightLayer.SKY, blockpos))) {
                                continue;
                            }
                        ...
                        if (entity instanceof Mob mob) {
                            if (!net.neoforged.neoforge.event.EventHooks.checkSpawnPositionSpawner(mob, pServerLevel, MobSpawnType.SPAWNER, spawndata, this)) {
                                continue;
                            }
                            ...
                            var event = net.neoforged.neoforge.event.EventHooks.onFinalizeSpawnSpawner(mob, pServerLevel, pServerLevel.getCurrentDifficultyAt(entity.blockPosition()), null, compoundtag, this);
    ...
    private void delay(Level pLevel, BlockPos pPos) {
        ...
        this.spawnPotentials.getRandom(randomsource).ifPresent(p_186386_ -> this.setNextSpawnData(pLevel, pPos, p_186386_.getData()));
