...
public class LootDataType<T> {
    ...
    /**
     * @deprecated Neo: use the constructor {@link #LootDataType(Codec, String,
     *             Validator, T, java.util.function.BiConsumer) with a default value
     *             and id setter} to support conditions
     */
    @Deprecated
    private LootDataType(Codec<T> pCodec, String pDirectory, LootDataType.Validator<T> pValidator) {
        this(pCodec, pDirectory, pValidator, null, (it, id) -> {});
    }
    ...
    public Optional<T> deserialize(ResourceLocation pLocation, JsonElement pJson) {
        return deserializeOrDefault(pLocation, JsonOps.INSTANCE, pJson);
    }
