...
public class ArmorTrim {
    ...
    public boolean equals(Object pOther) {
        if (!(pOther instanceof ArmorTrim)) {
            return false;
        } else {
            ArmorTrim armortrim = (ArmorTrim)pOther;
    ...
    public static Optional<ArmorTrim> getTrim(RegistryAccess pRegistryAccess, ItemStack pArmor, boolean pSkipLogging) {
        if (pArmor.is(ItemTags.TRIMMABLE_ARMOR) && pArmor.getTag() != null && pArmor.getTag().contains("Trim")) {
            ...
            ArmorTrim armortrim = CODEC.parse(RegistryOps.create(NbtOps.INSTANCE, pRegistryAccess), compoundtag).resultOrPartial(p_294057_ -> {
                if (!pSkipLogging) {
                    LOGGER.warn(p_294057_);
                }
            }).orElse(null);
    ...
    public static void appendUpgradeHoverText(ItemStack pArmor, RegistryAccess pRegistryAccess, List<Component> pTooltip) {
        Optional<ArmorTrim> optional = getTrim(pRegistryAccess, pArmor, true);
