...
public class TrimMaterials {
    ...
    public static Optional<Holder.Reference<TrimMaterial>> getFromIngredient(RegistryAccess pRegistryAccess, ItemStack pIngredient) {
        return pRegistryAccess.registryOrThrow(Registries.TRIM_MATERIAL).holders().filter(p_266876_ -> pIngredient.is(p_266876_.value().ingredient())).findFirst();
    }
    ...
    private static void register(
        BootstapContext<TrimMaterial> pContext,
        ResourceKey<TrimMaterial> pMaterialKey,
        Item pIngredient,
        Style pStyle,
        float pItemModelIndex,
        Map<ArmorMaterials, String> pOverrideArmorMaterials
    ) {
