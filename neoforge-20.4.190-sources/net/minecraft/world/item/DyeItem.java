...
public class DyeItem extends Item implements SignApplicator {
    ...
    public boolean tryApplyToSign(Level pLevel, SignBlockEntity pSign, boolean pIsFront, Player pPlayer) {
        if (pSign.updateText(p_277649_ -> p_277649_.setColor(this.getDyeColor()), pIsFront)) {
            pLevel.playSound(null, pSign.getBlockPos(), SoundEvents.DYE_USE, SoundSource.BLOCKS, 1.0F, 1.0F);
            return true;
        } else {
            return false;
        }
