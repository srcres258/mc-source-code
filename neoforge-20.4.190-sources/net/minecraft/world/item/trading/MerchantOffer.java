...
public class MerchantOffer {
    ...
    private MerchantOffer(MerchantOffer pOther) {
        this.baseCostA = pOther.baseCostA.copy();
        this.costB = pOther.costB.copy();
        this.result = pOther.result.copy();
        this.uses = pOther.uses;
        this.maxUses = pOther.maxUses;
        this.rewardExp = pOther.rewardExp;
        this.specialPriceDiff = pOther.specialPriceDiff;
        this.demand = pOther.demand;
        this.priceMultiplier = pOther.priceMultiplier;
        this.xp = pOther.xp;
    }
    ...
    public boolean satisfiedBy(ItemStack pPlayerOfferA, ItemStack pPlayerOfferB) {
        return this.isRequiredItem(pPlayerOfferA, this.getCostA())
            && pPlayerOfferA.getCount() >= this.getCostA().getCount()
            && this.isRequiredItem(pPlayerOfferB, this.costB)
            && pPlayerOfferB.getCount() >= this.costB.getCount();
    }
    ...
    private boolean isRequiredItem(ItemStack pOffer, ItemStack pCost) {
        if (pCost.isEmpty() && pOffer.isEmpty()) {
            ...
            return ItemStack.isSameItem(itemstack, pCost)
                && (!pCost.hasTag() || itemstack.hasTag() && NbtUtils.compareNbt(pCost.getTag(), itemstack.getTag(), false));
        }
    }
