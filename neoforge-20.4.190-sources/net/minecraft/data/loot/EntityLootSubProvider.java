...
public abstract class EntityLootSubProvider implements LootTableSubProvider {
                        ...
                        if (canHaveLootTable(entitytype)) {
                            ...
                            if (!resourcelocation.equals(BuiltInLootTables.EMPTY)
                                ...
                                throw new IllegalStateException(
                                    String.format(Locale.ROOT, "Missing loottable '%s' for '%s'", resourcelocation, p_266624_.key().location())
                                );
                            }
                                ...
                                map.forEach(
                                    (p_250376_, p_250972_) -> {
                                        if (!set.add(p_250376_)) {
                                            throw new IllegalStateException(
                                                String.format(Locale.ROOT, "Duplicate loottable '%s' for '%s'", p_250376_, p_266624_.key().location())
                                            );
                                        } else {
    ...
    protected LootItemCondition.Builder killedByFrogVariant(FrogVariant pFrogVariant) {
        return DamageSourceCondition.hasDamageSource(
            DamageSourcePredicate.Builder.damageType()
                .source(EntityPredicate.Builder.entity().of(EntityType.FROG).subPredicate(EntitySubPredicate.variant(pFrogVariant)))
        );
    }
    ...
    protected void add(EntityType<?> pEntityType, ResourceLocation pLootTableLocation, LootTable.Builder pBuilder) {
        this.map.computeIfAbsent(pEntityType, p_251466_ -> new HashMap()).put(pLootTableLocation, pBuilder);
    }
}
