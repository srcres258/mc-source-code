...
public class ModelProvider implements DataProvider {
    ...
    public CompletableFuture<?> run(CachedOutput pOutput) {
        ...
        if (!list.isEmpty()) {
            ...
            return CompletableFuture.allOf(
                this.saveCollection(pOutput, map, p_248016_ -> this.blockStatePathProvider.json(p_248016_.builtInRegistryHolder().key().location())),
    ...
    private <T> CompletableFuture<?> saveCollection(CachedOutput pOutput, Map<T, ? extends Supplier<JsonElement>> pObjectToJsonMap, Function<T, Path> pResolveObjectPath) {
        ...
        }).toArray(p_253409_ -> new CompletableFuture[p_253409_]));
    }
