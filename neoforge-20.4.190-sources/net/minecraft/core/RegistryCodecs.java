...
public class RegistryCodecs {
    private static <T> MapCodec<RegistryCodecs.RegistryEntry<T>> withNameAndId(ResourceKey<? extends Registry<T>> pRegistryKey, MapCodec<T> pElementCodec) {
        return RecordCodecBuilder.mapCodec(
            p_206309_ -> p_206309_.group(
                        ResourceKey.codec(pRegistryKey).fieldOf("name").forGetter(RegistryCodecs.RegistryEntry::key),
                        Codec.INT.fieldOf("id").forGetter(RegistryCodecs.RegistryEntry::id),
                        pElementCodec.forGetter(RegistryCodecs.RegistryEntry::value)
                    )
                    .apply(p_206309_, RegistryCodecs.RegistryEntry::new)
        );
    }
    ...
    public static <T> Codec<Registry<T>> networkCodec(ResourceKey<? extends Registry<T>> pRegistryKey, Lifecycle pLifecycle, Codec<T> pElementCodec) {
        return withNameAndId(pRegistryKey, pElementCodec.fieldOf("element")).codec().listOf().xmap(p_297933_ -> {
            MappedRegistry<T> mappedregistry = new MappedRegistry<>(pRegistryKey, pLifecycle);

            for(RegistryCodecs.RegistryEntry<T> registryentry : p_297933_) {
    ...
    public static <E> Codec<Registry<E>> fullCodec(ResourceKey<? extends Registry<E>> pRegistryKey, Lifecycle pLifecycle, Codec<E> pElementCodec) {
        // FORGE: Fix MC-197860
        Codec<Map<ResourceKey<E>, E>> codec = new net.neoforged.neoforge.common.LenientUnboundedMapCodec<>(ResourceKey.codec(pRegistryKey), pElementCodec);
        return codec.xmap(p_258184_ -> {
            WritableRegistry<E> writableregistry = new MappedRegistry<>(pRegistryKey, pLifecycle);
            p_258184_.forEach((p_258191_, p_258192_) -> writableregistry.register(p_258191_, p_258192_, pLifecycle));
