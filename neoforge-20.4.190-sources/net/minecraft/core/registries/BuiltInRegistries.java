...
public class BuiltInRegistries {
    ...
    private static <T> Registry<T> registerSimpleWithIntrusiveHolders(
        ResourceKey<? extends Registry<T>> pKey, BuiltInRegistries.RegistryBootstrap<T> pBootstrap
    ) {
        return internalRegister(pKey, new MappedRegistry<>(pKey, Lifecycle.stable(), true), pBootstrap, Lifecycle.stable());
    }
    ...
    private static <T> Registry<T> registerSimple(
        ResourceKey<? extends Registry<T>> pKey, Lifecycle pLifecycle, BuiltInRegistries.RegistryBootstrap<T> pBootstrap
    ) {
        return internalRegister(pKey, new MappedRegistry<>(pKey, pLifecycle, false), pBootstrap, pLifecycle);
    }

    private static <T> DefaultedRegistry<T> registerDefaulted(
        ResourceKey<? extends Registry<T>> pKey, String pDefaultKey, Lifecycle pLifecycle, BuiltInRegistries.RegistryBootstrap<T> pBootstrap
    ) {
    ...
    private static <T, R extends WritableRegistry<T>> R internalRegister(
        ResourceKey<? extends Registry<T>> pKey, R pRegistry, BuiltInRegistries.RegistryBootstrap<T> pBootstrap, Lifecycle pLifecycle
    ) {
