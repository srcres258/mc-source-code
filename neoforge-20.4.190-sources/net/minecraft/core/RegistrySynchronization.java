...
public class RegistrySynchronization {
    ...
    private static <E> void put(
        Builder<ResourceKey<? extends Registry<?>>, RegistrySynchronization.NetworkedRegistryData<?>> pNetworkableMapBuilder,
        ResourceKey<? extends Registry<E>> pKey,
        Codec<E> pNetworkCodec
    ) {
    ...
    private static Stream<RegistryAccess.RegistryEntry<?>> ownedNetworkableRegistries(RegistryAccess pRegistryAccess) {
        return pRegistryAccess.registries().filter(p_250129_ -> NETWORKABLE_REGISTRIES.containsKey(p_250129_.key()));
    }

    private static <E> DataResult<? extends Codec<E>> getNetworkCodec(ResourceKey<? extends Registry<E>> pRegistryKey) {
        return (DataResult<? extends Codec<E>>)Optional.ofNullable(NETWORKABLE_REGISTRIES.get(pRegistryKey))
            .map(p_250582_ -> (Codec<E>)p_250582_.networkCodec())
            .map(DataResult::success)
            .orElseGet(() -> DataResult.error(() -> "Unknown or not serializable registry: " + pRegistryKey));
    }
    ...
    private static <E> Codec<RegistryAccess> makeNetworkCodec() {
        ...
        Codec<Registry<E>> codec1 = codec.partialDispatch(
            "type",
            p_258198_ -> DataResult.success(p_258198_.key()),
            p_250682_ -> getNetworkCodec(p_250682_).map(p_252116_ -> RegistryCodecs.networkCodec(p_250682_, Lifecycle.experimental(), p_252116_))
        );
