...
public class ChunkRenamesFix extends DataFix {
    ...
    private static <T> Dynamic<T> mergeRemainders(Typed<?> pTyped, Dynamic<T> pDynamic) {
        ...
        DataResult<T> dataresult = dynamicops.getMap(pDynamic.getValue()).flatMap(p_199433_ -> dynamicops.mergeToMap(dynamic.getValue(), p_199433_));
        return dataresult.result().map(p_199436_ -> new Dynamic<>(dynamicops, p_199436_)).orElse(pDynamic);
    }
}
