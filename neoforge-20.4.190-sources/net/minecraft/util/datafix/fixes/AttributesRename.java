...
public class AttributesRename extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return TypeRewriteRule.seq(
            this.fixTypeEverywhereTyped("Rename ItemStack Attributes", type, p_14674_ -> p_14674_.updateTyped(opticfinder, AttributesRename::fixItemStackTag)),
    ...
    private static Dynamic<?> fixName(Dynamic<?> p_14678_) {
        return DataFixUtils.orElse(p_14678_.asString().result().map(p_14680_ -> RENAMES.getOrDefault(p_14680_, p_14680_)).map(p_14678_::createString), p_14678_);
    }

    private static Typed<?> fixItemStackTag(Typed<?> p_14676_) {
        return p_14676_.update(
            DSL.remainderFinder(),
            p_14694_ -> p_14694_.update(
                    "AttributeModifiers",
                    p_145080_ -> DataFixUtils.orElse(
                            p_145080_.asStreamOpt()
                                .result()
                                .map(p_145074_ -> p_145074_.map(p_145082_ -> p_145082_.update("AttributeName", AttributesRename::fixName)))
                                .map(p_145080_::createList),
                            p_145080_
                        )
                )
        );
    ...
    private static Typed<?> fixEntity(Typed<?> p_14684_) {
        return p_14684_.update(
            DSL.remainderFinder(),
            p_14686_ -> p_14686_.update(
                    "Attributes",
                    p_145076_ -> DataFixUtils.orElse(
                            p_145076_.asStreamOpt()
                                .result()
                                .map(p_145072_ -> p_145072_.map(p_145078_ -> p_145078_.update("Name", AttributesRename::fixName)))
                                .map(p_145076_::createList),
                            p_145076_
                        )
                )
        );
