...
public class VillagerTradeFix extends NamedEntityFix {
    ...
    protected Typed<?> fix(Typed<?> pTyped) {
        ...
        if (!(type instanceof ListType)) {
            ...
            OpticFinder<Pair<String, String>> opticfinder6 = DSL.fieldFinder(
                "id", DSL.named(References.ITEM_NAME.typeName(), NamespacedSchema.namespacedString())
            );
            Function<Typed<?>, Typed<?>> function = p_17150_ -> this.updateItemStack(opticfinder6, p_17150_);
            return pTyped.updateTyped(
                opticfinder,
                p_17125_ -> p_17125_.updateTyped(
                        opticfinder1,
                        p_145782_ -> p_145782_.updateTyped(
                                opticfinder2,
                                p_145788_ -> p_145788_.updateTyped(opticfinder3, function)
                                        .updateTyped(opticfinder4, function)
                                        .updateTyped(opticfinder5, function)
                            )
                    )
            );
        }
