...
public class StatsCounterFix extends DataFix {
    ...
    private TypeRewriteRule makeStatFixer() {
        ...
        return this.fixTypeEverywhereTyped("StatsCounterFix", type, type1, p_293829_ -> {
            Dynamic<?> dynamic = p_293829_.get(DSL.remainderFinder());
    ...
    private TypeRewriteRule makeObjectiveFixer() {
        ...
        return this.fixTypeEverywhereTyped(
            ...
            type1,
            p_311567_ -> {
                Dynamic<?> dynamic = p_311567_.get(DSL.remainderFinder());
                Dynamic<?> dynamic1 = dynamic.update(
                    "CriteriaName",
                    p_293832_ -> DataFixUtils.orElse(
                            p_293832_.asString()
                                .result()
                                .map(
                                    p_293833_ -> {
                                        if (SPECIAL_OBJECTIVE_CRITERIA.contains(p_293833_)) {
                                            return p_293833_;
                                        } else {
                                            StatsCounterFix.StatType statscounterfix$stattype = unpackLegacyKey(p_293833_);
