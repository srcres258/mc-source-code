...
public class ChunkHeightAndBiomeFix extends DataFix {
            ...
            p_184879_ -> p_184879_.updateTyped(
                    ...
                    p_184884_ -> {
                        ...
                        p_184884_ = p_184884_.updateTyped(opticfinder1, type3, p_184936_ -> {
                            ...
                            Dynamic<?> dynamic3 = p_184936_.write().result().orElseThrow(() -> new IllegalStateException("Malformed Chunk.Level.Sections"));
                            List<Dynamic<?>> list = dynamic3.asStream().map(p_184927_ -> {
                                ...
                                Dynamic<?> dynamic5 = DataFixUtils.orElse(p_184927_.get("Palette").result().flatMap(p_184940_ -> {
                                    ...
                                    return p_184927_.get("BlockStates").result().map(p_184973_ -> makeOptimizedPalettedContainer(p_184940_, p_184973_));
                                }), dynamic2);
    ...
    private static Dynamic<?>[] getBiomeContainers(Dynamic<?> pDynamic, boolean pOverworld, int p_184909_, MutableBoolean p_184910_) {
        ...
        int[] aint = (int[])pDynamic.get("Biomes").asIntStreamOpt().result().map(IntStream::toArray).orElse(null);
        if (aint != null && aint.length == 1536) {
            p_184910_.setValue(true);

            for(int l = 0; l < 24; ++l) {
                int l_f = l;
                dynamic[l] = makeBiomeContainer(pDynamic, p_184967_ -> getOldBiome(aint, l_f * 64 + p_184967_));
            }
        } else if (aint != null && aint.length == 1024) {
            for(int i = 0; i < 16; ++i) {
                int i_f = i;
                int j = i - p_184909_;
                dynamic[j] = makeBiomeContainer(pDynamic, p_184954_ -> getOldBiome(aint, i_f * 64 + p_184954_));
            }

            if (pOverworld) {
                Dynamic<?> dynamic1 = makeBiomeContainer(pDynamic, p_184976_ -> getOldBiome(aint, p_184976_ % 16));
                Dynamic<?> dynamic2 = makeBiomeContainer(pDynamic, p_184963_ -> getOldBiome(aint, p_184963_ % 16 + 1008));
    ...
    private static Dynamic<?> updateChunkTag(
        Dynamic<?> pChunkTag, boolean pOverworld, boolean p_184914_, boolean pIsNoiseGenerator, Supplier<ChunkProtoTickListFix.PoorMansPalettedContainer> p_184916_
    ) {
                ...
                if (optional.isPresent()) {
                    ...
                    if (!"empty".equals(s)) {
                        pChunkTag = pChunkTag.set(
                            "blending_data",
                            pChunkTag.createMap(
                                ImmutableMap.of(pChunkTag.createString("old_noise"), pChunkTag.createBoolean(STATUS_IS_OR_AFTER_NOISE.contains(s)))
                            )
                        );
    ...
    private static Dynamic<?> makeBiomeContainer(Dynamic<?> pDynamic, Int2IntFunction p_184896_) {
        ...
        Dynamic<?> dynamic = pDynamic.createList(
            int2intmap.keySet().stream().map(p_196598_ -> pDynamic.createString(BIOMES_BY_ID.getOrDefault(p_196598_.intValue(), "minecraft:plains")))
        );
