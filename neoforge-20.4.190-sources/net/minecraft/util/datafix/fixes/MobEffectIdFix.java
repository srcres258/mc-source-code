...
public class MobEffectIdFix extends DataFix {
    ...
    private static <T> Optional<Dynamic<T>> getAndConvertMobEffectId(Dynamic<T> pDynamic, String pKey) {
        return pDynamic.get(pKey).asNumber().result().map(p_298383_ -> ID_MAP.get(p_298383_.intValue())).map(pDynamic::createString);
    }
    ...
    private static <T> Dynamic<T> updateMobEffectInstance(Dynamic<T> p_298320_) {
        ...
        p_298320_ = renameField(p_298320_, "ShowParticles", "show_particles");
        p_298320_ = renameField(p_298320_, "ShowIcon", "show_icon");
        p_298320_ = renameField(p_298320_, "FactorCalculationData", "factor_calculation_data");
        Optional<Dynamic<T>> optional = p_298320_.get("HiddenEffect").result().map(MobEffectIdFix::updateMobEffectInstance);
        return replaceField(p_298320_, "HiddenEffect", "hidden_effect", optional);
    }
    ...
    private static <T> Dynamic<T> updateMobEffectInstanceList(Dynamic<T> pTag, String pOldName, String pNewName) {
        Optional<Dynamic<T>> optional = pTag.get(pOldName)
            .asStreamOpt()
            .result()
            .map(p_298291_ -> pTag.createList(p_298291_.map(MobEffectIdFix::updateMobEffectInstance)));
    ...
    private static <T> Dynamic<T> updateSuspiciousStewEntry(Dynamic<T> pOldDynamic, Dynamic<T> pNewDynamic) {
        pNewDynamic = updateMobEffectIdFieldConsideringForge(pOldDynamic, "EffectId", pNewDynamic, "id", "forge:effect_id");
    ...
    private Typed<?> updateNamedChoice(Typed<?> pTyped, TypeReference pReference, String pId, Function<Dynamic<?>, Dynamic<?>> pFixer) {
        ...
        return pTyped.updateTyped(DSL.namedChoice(pId, type), type1, p_298322_ -> p_298322_.update(DSL.remainderFinder(), pFixer));
    }
    ...
    private static <T> Dynamic<T> fixSuspiciousStewTag(Dynamic<T> p_298546_) {
        ...
        return replaceField(p_298546_, "Effects", "effects", optional);
    }
