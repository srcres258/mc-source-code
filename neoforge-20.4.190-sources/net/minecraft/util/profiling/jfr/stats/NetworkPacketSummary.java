    ...
    public static record PacketIdentification(PacketFlow direction, String protocolId, int packetId) {
        ...
        public static NetworkPacketSummary.PacketIdentification from(RecordedEvent pEvent) {
            return new NetworkPacketSummary.PacketIdentification(
                pEvent.getEventType().getName().equals("minecraft.PacketSent") ? PacketFlow.CLIENTBOUND : PacketFlow.SERVERBOUND,
                pEvent.getString("protocolId"),
                pEvent.getInt("packetId")
            );
        }
            ...
            for(ConnectionProtocol connectionprotocol : ConnectionProtocol.values()) {
                for(PacketFlow packetflow : PacketFlow.values()) {
                    ...
                    int2objectmap.forEach(
                        (p_293843_, p_293844_) -> builder.put(
                                new NetworkPacketSummary.PacketIdentification(packetflow, connectionprotocol.id(), p_293843_), p_293844_.getSimpleName()
                            )
                    );
                }
            }
